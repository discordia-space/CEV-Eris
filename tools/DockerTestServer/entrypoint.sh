...
# =================
# What this will do:
# 1) Copy the default config/ files from the read-only volume to /cev_eris/config/
# 2) Override files under /cev_eris/config/ with files present in /gamecfg/
# 3) Process environment variables passed to the container into /cev_eris/config/
...
sed -i -r 's/(#|^)ADDRESS .*/ADDRESS '"$DB_HOST"'/' /cev_eris/config/dbconfig.txt
sed -i -r 's/(#|^)FEEDBACK_LOGIN .*/FEEDBACK_LOGIN '"$DB_USER"'/' /cev_eris/config/dbconfig.txt
sed -i -r 's/(#|^)FEEDBACK_PASSWORD .*/FEEDBACK_PASSWORD '"$DB_PASS"'/' /cev_eris/config/dbconfig.txt
...
envvar_override "TG_GAME_" "/cev_eris/config/game_options.txt"
...
cd /cev_eris
DreamDaemon cev_eris.dmb -port 1337 -trusted -close -verbose
