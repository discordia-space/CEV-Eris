...
/client/proc/asset_cache_update_json()
	...
	UPDATE_ASSETS(src, json_encode(sent_assets), "file=asset_data.json&display=0")
...
/client/proc/browse_queue_flush(timeout = 50)
	...
	src << browse({"<script>window.location.href="?asset_cache_confirm_arrival=[job]"</script>"}, "window=asset_cache_browser&file=asset_cache_send_verify.htm") //unmacro'd due to formatting limitations
