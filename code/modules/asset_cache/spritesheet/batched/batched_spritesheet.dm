...
/datum/asset/spritesheet_batched/proc/realize_spritesheets(yield)
	...
	if(yield || !isnull(job_id))
		if(isnull(job_id))
			job_id = rustg_iconforge_generate_async("data/spritesheets/", name, entries_json, do_cache, TRUE, TRUE)
		UNTIL((data_out = rustg_iconforge_check(job_id)) != RUSTG_JOB_NO_RESULTS_YET)
	else
		data_out = rustg_iconforge_generate("data/spritesheets/", name, entries_json, do_cache, TRUE, TRUE)
	if (data_out == RUSTG_JOB_ERROR)
