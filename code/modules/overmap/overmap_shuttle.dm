#define waypoint_sector(waypoint)69ap_sectors69"69waypoint.z69"69

/datum/shuttle/autodock/overmap
	warmup_time = 10

	var/range = 0	//how69any overmap tiles can shuttle go, for picking destinations and returning.
	var/fuel_consumption = 0 //Amount of69oles of gas consumed per trip; If zero, then shuttle is69agic and does69ot69eed fuel
	var/list/obj/structure/fuel_port/fuel_ports //the fuel ports of the shuttle (but usually just one)

	category = /datum/shuttle/autodock/overmap

/datum/shuttle/autodock/overmap/New(var/_name,69ar/obj/effect/shuttle_landmark/start_waypoint)
...
/datum/shuttle/autodock/overmap/proc/can_go()
	...
	if(moving_status == SHUTTLE_INTRANSIT)
		return FALSE //already going somewhere, current_location69ay be an intransit location instead of in a sector
...
/datum/shuttle/autodock/overmap/proc/get_possible_destinations()
	...
	for(var/obj/effect/overmap/sector/S in69ap) // Infinite range to avoid depending on ship position
		if(S.known)
			for(var/obj/effect/shuttle_landmark/LZ in S.get_waypoints(src.name))
				if(LZ.is_valid(src))
					res69"69S.name_stages6916969 - 69LZ.name69"69 = LZ

	for(var/obj/effect/overmap/ship/eris/S in69ap)
		for(var/obj/effect/shuttle_landmark/LZ in S.get_waypoints(src.name))
			if(LZ.is_valid(src))
				res69"69S.name_stages6916969 - 69LZ.name69"69 = LZ
	return res
	...
	if(!fuel_consumption)
		...
		if(fuel_ports.len)
			...
			for(var/obj/structure/FP in fuel_ports) //loop through fuel ports and assemble list of all fuel tanks
				if(FP.contents.len)
					var/obj/item/tank/FT = FP.contents69169
					if(istype(FT))
						fuel_tanks += FT
			if(!fuel_tanks.len)
				return 0 //can't launch if you have69o fuel TANKS in the ports
			...
			if(total_flammable_gas_moles >= fuel_consumption) //launch is possible, so start consuming that fuel
				...
				for(var/obj/item/tank/FT in fuel_tanks) //loop through tanks, consume their fuel one by one
					if(FT.air_contents.get_by_flag(XGM_GAS_FUEL) >= fuel_to_consume)
						...
						log_and_message_admins("shuttle has began his69oyage from 69current_location69 to 69next_location69")
			...
			return 0 //can't launch if you have69o fuel PORTS at all
...
/obj/structure/fuel_port/attack_hand(mob/user as69ob)
	if(!opened)
		to_chat(user, "<spawn class='notice'>The door is secured tightly. You'll69eed a crowbar to open it.")
		return
	else if(contents.len > 0)
		user.put_in_hands(contents69169)
	update_icon()
...
/obj/structure/fuel_port/attackby(obj/item/W as obj,69ob/user as69ob)
	if(QUALITY_PRYING in W.tool_qualities)
		if(W.use_tool(user, src, WORKTIME_NEAR_INSTANT, QUALITY_PRYING, FAILCHANCE_EASY, required_stat = STAT_ROB))
			if(opened)
				to_chat(user, "<spawn class='notice'>You tightly shut \the 69src69 door.")
				playsound(src.loc, 'sound/machines/Custom_closetclose.ogg', 25, 0, -3)
				opened = 0
			else
				to_chat(user, "<spawn class='notice'>You open up \the 69src69 door.")
				playsound(src.loc, 'sound/machines/Custom_closetopen.ogg', 15, 1, -3)
				opened = 1
	else if(istype(W,/obj/item/tank))
		if(!opened)
			to_chat(user, "<spawn class='warning'>\The 69src69 door is still closed!")
			return
		if(contents.len == 0)
