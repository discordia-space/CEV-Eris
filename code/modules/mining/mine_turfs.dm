...
/turf/simulated/mineral/attackby(obj/item/I, mob/living/user)
	...
	switch(tool_type)
		...
		if(QUALITY_DIGGING)
			...
			if(I.use_tool(user, src, WORKTIME_NORMAL, tool_type, FAILCHANCE_VERY_EASY, required_stat = STAT_STR))
...
/turf/simulated/floor/asteroid/attackby(obj/item/I, mob/user)
	...
	if(QUALITY_DIGGING in I.tool_qualities)
		if (dug)
			...
			return
		if(I.use_tool(user, src, WORKTIME_NORMAL, QUALITY_DIGGING, FAILCHANCE_EASY, required_stat = STAT_STR))
