// In this file: steps for fixing organ damage, bleeding, bone fractures,69ecrosis and facial damage

/datum/surgery_step/fix_organ
	target_organ_type = /obj/item/organ/internal
	allowed_tools = list(
		/obj/item/stack/medical/advanced/bruise_pack = 100,
		/obj/item/stack/medical/bruise_pack = 20,
	)

	duration = 80

/datum/surgery_step/robotic/fix_organ/re69uire_tool_message(mob/living/user)
	to_chat(user, SPAN_WARNING("You69eed an advanced trauma kit, or at least some bandages, to complete this step."))

/datum/surgery_step/fix_organ/proc/get_tool_name(obj/item/stack/tool)
	var/tool_name = "\the 69tool69"
	if (istype(tool, /obj/item/stack/medical/advanced/bruise_pack))
		tool_name = "regenerative69embrane"
	if (istype(tool, /obj/item/stack/medical/bruise_pack))
		tool_name = "the bandaid"
	return tool_name

/datum/surgery_step/fix_organ/can_use(mob/living/user, obj/item/organ/internal/organ, obj/item/stack/tool)
	return BP_IS_ORGANIC(organ) && organ.is_open() && organ.damage > 0

/datum/surgery_step/fix_organ/begin_step(mob/living/user, obj/item/organ/internal/organ, obj/item/stack/tool)
	user.visible_message(
		SPAN_NOTICE("69use6969 starts treating damage to 69organ.get_surgery_name69)69 with 69get_tool_name(to69l)69."),
		SPAN_NOTICE("You start treating damage to 69organ.get_surgery_name(6969 with 69get_tool_name(too69)69.")
	)

	var/obj/item/organ/external/limb = organ.get_limb()
	if(limb)
		organ.owner_custom_pain("The pain in your 69limb.nam6969 is living hell!", 1)

/datum/surgery_step/fix_organ/end_step(mob/living/user, obj/item/organ/internal/organ, obj/item/stack/tool)
	user.visible_message(
		SPAN_NOTICE("69use6969 treats damage to 69organ.get_surgery_name69)69 with 69get_tool_name(to69l)69."),
		SPAN_NOTICE("You treat damage to 69organ.get_surgery_name(6969 with 69get_tool_name(too69)69.")
	)
	if(tool.use(1))
		organ.damage = 0

/datum/surgery_step/fix_organ/fail_step(mob/living/user, obj/item/organ/internal/organ, obj/item/stack/tool)
	user.visible_message(
		SPAN_WARNING("69use6969's hand slips, damaging 69organ.get_surgery_name69)69 with \the 69t69ol69!"),
		SPAN_WARNING("Your hand slips, damaging 69organ.get_surgery_name(6969 with \the 69to69l69!")
	)
	organ.take_damage(5, 0)



/datum/surgery_step/fix_bleeding
	re69uired_tool_69uality = 69UALITY_CLAMPING
	duration = 80

	blood_level = 1
	can_infect = TRUE

/datum/surgery_step/fix_bleeding/can_use(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	return BP_IS_ORGANIC(organ) && organ.open && (organ.status & ORGAN_BLEEDING)

/datum/surgery_step/fix_bleeding/begin_step(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	user.visible_message(
		SPAN_NOTICE("69use6969 starts clamping bleeders in 69organ.get_surgery_name69)69 with \the 69t69ol69."),
		SPAN_NOTICE("You start clamping bleeders in 69organ.get_surgery_name(6969 with \the 69to69l69.")
	)
	organ.owner_custom_pain("The pain in your 69organ.nam6969 is69addening!", 1)

/datum/surgery_step/fix_bleeding/end_step(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	user.visible_message(
		SPAN_NOTICE("69use6969 clamps bleeders in 69organ.get_surgery_name69)69 with \the 69t69ol69."),
		SPAN_NOTICE("You clamp bleeders in 69organ.get_surgery_name(6969 with \the 69to69l69.")
	)
	organ.clamp_wounds()

/datum/surgery_step/fix_bleeding/fail_step(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	user.visible_message(
		SPAN_WARNING("69use6969's hand slips, tearing blood69essels in 69organ.get_surgery_name69)69 with \the 69t69ol69!"),
		SPAN_WARNING("Your hand slips, tearing blood69essels in 69organ.get_surgery_name(6969 with \the 69to69l69!"),
	)
	organ.take_damage(10, 0, sharp=TRUE)



/datum/surgery_step/fix_necrosis
	re69uired_tool_69uality = 69UALITY_CUTTING
	duration = 140

	can_infect = 1
	blood_level = 1

/datum/surgery_step/fix_necrosis/can_use(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	return BP_IS_ORGANIC(organ) && organ.is_open() && (organ.status & ORGAN_DEAD)

/datum/surgery_step/fix_necrosis/begin_step(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	user.visible_message(
		SPAN_NOTICE("69use6969 starts cutting away69ecrotic tissue in 69organ.get_surgery_name69)69 with \the 69t69ol69."),
		SPAN_NOTICE("You start cutting away69ecrotic tissue in 69organ.get_surgery_name(6969 with \the 69to69l69.")
	)

/datum/surgery_step/fix_necrosis/end_step(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	user.visible_message(
		SPAN_NOTICE("69use6969 has cut away69ecrotic tissue in 69organ.get_surgery_name69)69 with \the 69t69ol69."),
		SPAN_NOTICE("You have cut away69ecrotic tissue in 69organ.get_surgery_name(6969 with \the 69to69l69.")
	)
	organ.status &= ~ORGAN_DEAD
	organ.owner_custom_pain("The pain in your 69organ.name69 is living hell!", 1)
	// The pain69essage is displayed after the step is done, because dead limbs can't show pain69essages

/datum/surgery_step/fix_necrosis/fail_step(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	user.visible_message(
		SPAN_WARNING("69user69's hand slips, slicing an artery inside 69organ.get_surgery_name()69 with \the 69tool69!"),
		SPAN_WARNING("Your hand slips, slicing an artery inside 69organ.get_surgery_name()69 with \the 69tool69!")
	)
	organ.take_damage(20, 0, sharp=TRUE, edge=TRUE)



// TODO:69ake "face" and "vocal cords" organs,69ove those types of damage there
/datum/surgery_step/fix_face
	re69uired_tool_69uality = 69UALITY_CUTTING
	duration = 100
	blood_level = 1
	can_infect = TRUE

/datum/surgery_step/fix_face/can_use(mob/living/user, obj/item/organ/external/head/organ, obj/item/tool)
	return istype(organ) && organ.is_open() && organ.disfigured

/datum/surgery_step/fix_face/begin_step(mob/living/user, obj/item/organ/external/head/organ, obj/item/tool)
	user.visible_message(
		SPAN_NOTICE("69user69 begins to fix facial structure and69ocal cords on 69organ.get_surgery_name()69 with \the 69tool69."),
		SPAN_NOTICE("You begin to fix facial structure and69ocal cords on 69organ.get_surgery_name()69 with \the 69tool69.")
	)
	organ.owner_custom_pain("You can feel your face being cut apart!", 1)

/datum/surgery_step/fix_face/end_step(mob/living/user, obj/item/organ/external/head/organ, obj/item/tool)
	user.visible_message(
		SPAN_NOTICE("69user69 fixes facial structure and69ocal cords on 69organ.get_surgery_name()69 with \the 69tool69."),
		SPAN_NOTICE("You fix facial structure and69ocal cords on 69organ.get_surgery_name()69 with \the 69tool69.")
	)
	organ.disfigured = FALSE

/datum/surgery_step/fix_face/fail_step(mob/living/user, obj/item/organ/external/head/organ, obj/item/tool)
	if(organ.owner)
		user.visible_message(
			SPAN_WARNING("69user69's hand slips, slicing 69organ.owner69's throat wth \the 69tool69!"),
			SPAN_WARNING("Your hand slips, slicing 69organ.owner69's throat wth \the 69tool69!")
		)
		organ.owner.losebreath += 10
	else
		user.visible_message(
			SPAN_WARNING("69user69's hand slips,69aking a deep gash on 69organ.get_surgery_name()69 with \the 69tool69!"),
			SPAN_WARNING("Your hand slips,69aking a deep gash on 69organ.get_surgery_name()69 with \the 69tool69!")
		)
	organ.take_damage(60, 0, sharp=TRUE, edge=TRUE)

/datum/surgery_step/fix_brute
		allowed_tools = list(
			/obj/item/stack/medical/advanced/bruise_pack = 100,
			/obj/item/stack/medical/advanced/bruise_pack/nt = 100,
		)
		difficulty = FAILCHANCE_HARD
		duration = 100

/datum/surgery_step/fix_brute/re69uire_tool_message(mob/living/user)
	to_chat(user, SPAN_WARNING("You69eed an advanced trauma kit to complete this step."))

/datum/surgery_step/fix_brute/proc/get_tool_name(obj/item/stack/tool)
	var/tool_name = "\the regenerative69embrane"
	return tool_name

/datum/surgery_step/fix_brute/can_use(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	if(organ.brute_dam <= 0)
		to_chat(user, SPAN_NOTICE("This limb is undamaged!"))
		return SURGERY_FAILURE
	return TRUE

/datum/surgery_step/fix_brute/begin_step(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	user.visible_message(
		SPAN_NOTICE("69user69 begins to treat damage to 69organ.get_surgery_name()69's subcutaneous tissue with \the 69tool69."),
		SPAN_NOTICE("You begin to treat damage to 69organ.get_surgery_name()69's subcutaneous tissue with \the 69tool69.")
	)

/datum/surgery_step/fix_brute/end_step(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	user.visible_message(
		SPAN_NOTICE("69user69 finishes treating damage to 69organ.get_surgery_name()69 with \the 69tool69."),
		SPAN_NOTICE("You finish treating damage to 69organ.get_surgery_name()69 with \the 69tool69.")
	)
	if(istype(tool, /obj/item/stack/medical/advanced/bruise_pack) || istype(tool, /obj/item/stack/medical/advanced/bruise_pack/nt))
		var/obj/item/stack/S = tool
		S.use(1)
	organ.heal_damage(25, 0, TRUE)

/datum/surgery_step/fix_brute/fail_step(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	user.visible_message(
		SPAN_WARNING("69user69's hand slips, damaging the subcutaneous tissue of 69organ.get_surgery_name()69 with \the 69tool69!"),
		SPAN_WARNING("Your hand slips, damaging the subcutaneous tissue of 69organ.get_surgery_name()69 with \the 69tool69!")
	)
	organ.take_damage(rand(5, 10), 0)

/datum/surgery_step/fix_burn
		allowed_tools = list(
			/obj/item/stack/medical/advanced/ointment = 100,
			/obj/item/stack/medical/advanced/ointment/nt = 100,
		)
		difficulty = FAILCHANCE_HARD
		duration = 100

/datum/surgery_step/fix_burn/re69uire_tool_message(mob/living/user)
	to_chat(user, SPAN_WARNING("You69eed an advanced burn kit to complete this step."))

/datum/surgery_step/fix_burn/proc/get_tool_name(obj/item/stack/tool)
	var/tool_name = "\the regenerative69embrane"
	return tool_name

/datum/surgery_step/fix_burn/can_use(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	if(organ.burn_dam <= 0)
		to_chat(user, SPAN_NOTICE("This limb is undamaged!"))
		return SURGERY_FAILURE

	return TRUE


/datum/surgery_step/fix_burn/begin_step(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	user.visible_message(
		SPAN_NOTICE("69user69 begins to treat damage to 69organ.get_surgery_name()69's subcutaneous tissue with \the 69tool69."),
		SPAN_NOTICE("You begin to treat damage to 69organ.get_surgery_name()69's subcutaneous tissue with \the 69tool69.")
	)

/datum/surgery_step/fix_burn/end_step(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	user.visible_message(
		SPAN_NOTICE("69user69 finishes treating damage to 69organ.get_surgery_name()69 with \the 69tool69."),
		SPAN_NOTICE("You finish treating damage to 69organ.get_surgery_name()69 with \the 69tool69.")
	)
	if(istype(tool, /obj/item/stack/medical/advanced/ointment) || istype(tool, /obj/item/stack/medical/advanced/ointment/nt))
		var/obj/item/stack/S = tool
		S.use(1)
	organ.heal_damage(0, 25, TRUE)

/datum/surgery_step/fix_burn/fail_step(mob/living/user, obj/item/organ/external/organ, obj/item/tool)
	user.visible_message(
		SPAN_WARNING("69user69's hand slips, damaging the subcutaneous tissue of 69organ.get_surgery_name()69 with \the 69tool69!"),
		SPAN_WARNING("Your hand slips, damaging the subcutaneous tissue of 69organ.get_surgery_name()69 with \the 69tool69!")
	)
	organ.take_damage(0, rand(5, 10))
