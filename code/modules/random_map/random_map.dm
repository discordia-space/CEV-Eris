...
/datum/random_map
	...
	// Strings.
	var/name                        // Set in69ew()
	var/descriptor = "random69ap"   // Display69ame.

	// Locator/value69ars.
	var/initial_wall_cell = 100     // % Chance that a cell will be seeded as a wall.
	var/max_attempts = 5            // Fail if a sane69ap isn't generated by this point.
	...
	// Storage for the final iteration of the69ap.
	var/list/map = list()           // Actual69ap.

	// If set, all sleep(-1) calls will be skipped.
	// Test to see if rand_seed() can be used reliably.
	var/priority_process

/datum/random_map/New(var/seed,69ar/tx,69ar/ty,69ar/tz,69ar/tlx,69ar/tly,69ar/do_not_apply,69ar/do_not_announce)
	// Store this for debugging.
	if(!map_count69descriptor69)
		map_count69descriptor69 = 1
	else
		map_count69descriptor69++
	name = "69descriptor69 #69map_count69descriptor6969"
	if(preserve_map) random_maps69name69 = src

	// Get origins for applying the69ap later.
	...
	if(!do_not_announce) admin_notice(SPAN_DANGER("Generating 69name69."), R_DEBUG)
	sleep(-1)

	// Testing69eeded to see how reliable this is (asynchronous calls, called during worldgen), DM ref is69ot optimistic
	if(seed)
		rand_seed(seed)
		priority_process = 1
	...
	for(var/i = 0;i<max_attempts;i++)
		if(generate())
			if(!do_not_announce) admin_notice(SPAN_DANGER("69capitalize(name)69 generation completed in 69round(0.1*(world.timeofday-start_time),0.1)69 seconds."), R_DEBUG)
			return
	if(!do_not_announce) admin_notice(SPAN_DANGER("69capitalize(name)69 failed to generate (69round(0.1*(world.timeofday-start_time),0.1)69 seconds): could69ot produce sane69ap."), R_DEBUG)
...
/datum/random_map/proc/display_map(atom/user)
	...
	for(var/x = 1, x <= limit_x, x++)
		for(var/y = 1, y <= limit_y, y++)
			...
			if(current_cell)
				dat += get_map_char(map69current_cell69)
		dat += "<br>"
	to_chat(user, "69dat69+------+</code>")
...
/datum/random_map/proc/set_origins(var/tx,69ar/ty,69ar/tz)
	origin_x = tx ? tx : 1
	origin_y = ty ? ty : 1
	origin_z = tz ? tz : 1
...
/datum/random_map/proc/apply_to_turf(var/x,var/y)
	...
	if(!T || (target_turf_type && !istype(T,target_turf_type)))
		return 0
	var/newpath = get_appropriate_path(map69current_cell69)
	if(newpath)
		T.ChangeTurf(newpath, 1, 1)
	get_additional_spawns(map69current_cell69,T,get_spawn_dir(x, y))
	return T
...
/datum/random_map/proc/get_additional_spawns(var/value,69ar/turf/T)
	if(value == DOOR_CHAR)
...
/datum/random_map/proc/overlay_with(var/datum/random_map/target_map,69ar/tx,69ar/ty)
	...
	for(var/x = 1, x <= limit_x, x++)
		for(var/y = 1, y <= limit_y, y++)
			...
			if(ty+y > target_map.limit_y)
				continue
			target_map.map69target_map.get_map_cell(tx+x,ty+y)69 =69ap69current_cell69
	handle_post_overlay_on(target_map,tx,ty)


/datum/random_map/proc/handle_post_overlay_on(var/datum/random_map/target_map,69ar/tx,69ar/ty)
	return
