// Simple datum to keep track of a running holomap. Each69achine capable of displaying the holomap will have one.
...
/datum/station_holomap/proc/initialize_holomap(turf/T, isAI = null,69ob/user = null, reinit = FALSE)
	if(!station_map || reinit)
		station_map = image(SSholomaps.extraMiniMaps69"69HOLOMAP_EXTRA_STATIONMAP69_69T.z69"69)
	if(!cursor || reinit)
	...
	if(isStationLevel(T.z))
		if(!deck_name || reinit)
			...
			for(var/level in GLOB.maps_data.station_levels)
				deck_name = image(HOLO_DECK_NAME, "deck69level69")
				if(ISODD(level))
...
// /datum/station_holomap/strategic/initialize_holomap(var/turf/T,69ar/isAI=null,69ar/mob/user=null)
// 	..()
// 	station_map = image(SSholomaps.extraMiniMaps69HOLOMAP_EXTRA_STATIONMAP_STRATEGIC69)
