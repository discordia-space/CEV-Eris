/*This code for boxes with ammo, you cant use them as69agazines, but should be able to fill69agazines with them.*/
/obj/item/ammo_magazine/ammobox	//Should69ot be used bu its own
	name = "ammunition box"
	desc = "Gun ammunition stored in a shiny69ew box. You can see caliber information on the label."
	mag_type = SPEEDLOADER	//To prevent load in69agazine filled guns
...
/obj/item/ammo_magazine/ammobox/resolve_attackby(atom/A,69ob/user)
	if(isturf(A) && locate(/obj/item/ammo_casing) in A || istype(A, /obj/item/ammo_casing))
		if(!do_after(user, src.reload_delay, src))
			to_chat(user, SPAN_WARNING("You stoped scooping ammo into 69src69."))
			return
		if(collectAmmo(get_turf(A), user))
			return TRUE
	..()

/obj/item/ammo_magazine/ammobox/proc/collectAmmo(turf/target,69ob/user)
	...
	if(user)
		if(.)
			user.visible_message(SPAN_NOTICE("69user69 scoopes some ammo in 69src69."),SPAN_NOTICE("You scoop some ammo in 69src69."),SPAN_NOTICE("You hear69etal clanging."))
		else
...
/obj/item/ammo_magazine/ammobox/pistol
	...
	icon_state = "pistol"
	matter = list(MATERIAL_STEEL = 10,69ATERIAL_CARDBOARD = 1)
	caliber = CAL_PISTOL
...
/obj/item/ammo_magazine/ammobox/magnum
	name = "ammunition packet (.4069agnum)"
	icon_state = "magnum"
	matter = list(MATERIAL_STEEL = 9,69ATERIAL_CARDBOARD = 1)
	caliber = CAL_MAGNUM
...
/obj/item/ammo_magazine/ammobox/srifle_small
	...
	icon_state = "srifle"
	matter = list(MATERIAL_STEEL = 20,69ATERIAL_CARDBOARD = 1)
	caliber = CAL_SRIFLE
	ammo_type = /obj/item/ammo_casing/srifle
	max_ammo = 60
	ammo_states = list(60)
...
/obj/item/ammo_magazine/ammobox/lrifle
	...
	mag_type = SPEEDLOADER |69AGAZINE
	max_ammo = 240
	ammo_states = list(240)
...
/obj/item/ammo_magazine/ammobox/antim
	name = "ammunition box (.60 Anti69aterial)"
	icon_state = "antim"
