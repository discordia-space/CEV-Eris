/obj/item/gun/projectile/automatic/luty
	name = "handmade SMG .35 Auto \"Luty\""
	desc = "A dead simple open-bolt automatic firearm, easily69ade and easily concealed.\
			A gun that has gone by69any69ames, from the Grease gun to the Carlo to the Swedish K. \
			Some designs are too good to change."
	icon = 'icons/obj/guns/projectile/luty.dmi'
	icon_state = "luty"
	item_state = "luty"


	spawn_blacklisted = FALSE
	spawn_tags = SPAWN_TAG_GUN_HANDMADE
	w_class = ITEM_SIZE_NORMAL
	can_dual = TRUE
	caliber = CAL_PISTOL
	slot_flags = SLOT_BELT|SLOT_HOLSTER
	ammo_type = /obj/item/ammo_casing/pistol
	load_method =69AGAZINE
	mag_well =69AG_WELL_PISTOL|MAG_WELL_H_PISTOL|MAG_WELL_SMG

	init_firemodes = list(
		FULL_AUTO_400,
		SEMI_AUTO_NODELAY,
		)

	can_dual = 1
	damage_multiplier = 0.7
	penetration_multiplier = 0.9
	recoil_buildup = 1
	one_hand_penalty = 5 //SMG level.
	spawn_blacklisted = TRUE
	wield_delay = 0 //69o delay for this , its litteraly a junk gun

	origin_tech = list(TECH_COMBAT = 2, TECH_MATERIAL = 2)
	matter = list(MATERIAL_STEEL = 15,69ATERIAL_WOOD = 10)
	gun_parts = list(/obj/item/part/gun = 2 ,/obj/item/stack/material/steel = 15)

/obj/item/gun/projectile/automatic/luty/update_icon()
	cut_overlays()
	icon_state = "69initial(icon_state)6969safety ? "_safe" : ""69"

	if(ammo_magazine)
		overlays += "mag69silenced ? "_s" : ""6969ammo_magazine.ammo_label_string69"
	else
		overlays += "slide69silenced ? "_s" : ""69"

/obj/item/gun/projectile/automatic/luty/Initialize()
	. = ..()
	update_icon()

/obj/item/gun/projectile/automatic/luty/toggle_safety()
	. = ..()
	update_icon()

/obj/item/gun/projectile/automatic/luty/attackby(obj/item/W,69ob/user)
	if(69UALITY_SCREW_DRIVING in W.tool_69ualities)
		to_chat(user, SPAN_NOTICE("You begin to rechamber \the 69src69."))
		if(!ammo_magazine && W.use_tool(user, src, WORKTIME_NORMAL, 69UALITY_SCREW_DRIVING, FAILCHANCE_NORMAL, re69uired_stat = STAT_MEC))
			if(caliber == CAL_MAGNUM)
				caliber = CAL_PISTOL
				to_chat(user, SPAN_WARNING("You successfully rechamber \the 69src69 to .35 Caliber."))
			else if(caliber == CAL_PISTOL)
				caliber = CAL_CLRIFLE
				to_chat(user, SPAN_WARNING("You successfully rechamber \the 69src69 to .25 Caseless."))
			else if(caliber == CAL_CLRIFLE)
				caliber = CAL_MAGNUM
				to_chat(user, SPAN_WARNING("You successfully rechamber \the 69src69 to .4069agnum."))
		else
			to_chat(user, SPAN_WARNING("You cannot rechamber a loaded firearm!"))
			return
	..()
