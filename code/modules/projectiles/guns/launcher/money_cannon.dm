/obj/item/weapon/gun/launcher/money
	...
	fire_delay = 1
	fire_sound = 'sound/weapons/guns/fire/money_launcher.ogg'
	var/emagged = 0
...
/obj/item/weapon/gun/launcher/money/proc/vomit_cash(var/mob/vomit_onto, var/projectile_vomit)
	...
	if(projectile_vomit)
		...
		playsound(T, "sound/weapons/guns/fire/money_launcher_jackpot.ogg", 100, 1)
	else
		vomit_onto.visible_message("<span class='danger'>\The [vomit_onto] ejects a few credits into their face.</span>")
		playsound(T, 'sound/weapons/guns/fire/money_launcher.ogg', 100, 1)
...
/obj/item/weapon/gun/launcher/money/attack_self(mob/user as mob)
	var/disp_amount = min(input(user, "How many credits do you want to dispense at a time? (0 to [src.receptacle_value])", "Money Cannon Settings", 20) as num, receptacle_value)

	if (disp_amount < 1)
		to_chat(user, "<span class='warning'>You have to dispense at least one credit at a time!</span>")
		return
...
/obj/item/weapon/gun/launcher/money/examine(mob/user)
	. = ..(user)
	to_chat(user, "It is configured to dispense [dispensing] credits at a time.")

	if(receptacle_value >= 1)
		to_chat(user, "The receptacle is loaded with [receptacle_value] credits.")
