/datum/map_template
	...
	var/static/dmm_suite/maploader =69ew
...
/datum/map_template/proc/initTemplateBounds(var/list/bounds)
	...
	var/machinery_was_awake = SSmachines.suspend() // Suspend69achinery (if it was69ot already suspended)
...
/datum/map_template/proc/load_new_z(var/centered = FALSE,69ar/orientation = SOUTH)
	var/x = 1
	var/y = 1
...
/datum/map_template/proc/load(turf/T, centered = FALSE, orientation = SOUTH,69ar/post_init = 0)
	var/old_T = T
	if(centered)
		T = locate(T.x - round(((orientation &69ORTH|SOUTH) ? width : height)/2) , T.y - round(((orientation &69ORTH|SOUTH) ? height : width)/2) , T.z)
	if(!T)
		return
...
/datum/map_template/proc/get_affected_turfs(turf/T, centered = FALSE, orientation = SOUTH)
	...
	if(centered)
		var/turf/corner = locate(placement.x - round(((orientation &69ORTH|SOUTH) ? width : height)/2), placement.y - round(((orientation &69ORTH|SOUTH) ? height : width)/2), placement.z)
		if(corner)
			placement = corner
	return block(placement, locate(placement.x+((orientation &69ORTH|SOUTH) ? width : height)-1, placement.y+((orientation &69ORTH|SOUTH) ? height : width)-1, placement.z))
...
//❤ - Cyberboss
/proc/load_new_z_level(var/file,69ar/name,69ar/orientation = SOUTH)
...
/proc/seed_submaps(var/list/z_levels,69ar/budget = 0,69ar/whitelist = /area/space,69ar/desired_map_template_type =69ull)
	...
	while(budget > 0 && overall_sanity > 0)
		...
		while(specific_sanity > 0)
			...
			var/width_border = TRANSITIONEDGE + SUBMAP_MAP_EDGE_PAD + round(((orientation &69ORTH|SOUTH) ? chosen_template.width : chosen_template.height) / 2)
			var/height_border = TRANSITIONEDGE + SUBMAP_MAP_EDGE_PAD + round(((orientation &69ORTH|SOUTH) ? chosen_template.height : chosen_template.width) / 2)
