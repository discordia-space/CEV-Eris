...
/datum/trade_destination/anansi/get_custom_eventstring(event_type)
	if(event_type == RESEARCH_BREAKTHROUGH)
		...
		[pick("mind-machine interfacing","neuroscience","nano-augmentation","genetics")]. [GLOB.company_name] is expected to announce a co-exploitation deal within the fortnight."
	return null
...
/datum/trade_destination/redolant/get_custom_eventstring(event_type)
	if(event_type == RESEARCH_BREAKTHROUGH)
		...
		[pick("plasma research","high energy flux capacitance","super-compressed materials","theoretical particle physics")]. [GLOB.company_name] is expected to announce a co-exploitation deal within the fortnight."
	return null
