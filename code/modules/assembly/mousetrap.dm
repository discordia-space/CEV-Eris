...
/obj/item/device/assembly/mousetrap/proc/triggered(var/mob/living/target, var/type = "feet")
	...
	if(ismouse(target))
		...
		if (!isrobot(target))
			...
			target.damage_through_armor(rand(15,30), HALLOSS, zone, ARMOR_BLUNT, used_weapon = src)
			target.damage_through_armor(rand(8,15), BRUTE, zone, ARMOR_BLUNT, used_weapon = src)
