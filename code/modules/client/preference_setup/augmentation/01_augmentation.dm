...
/datum/preferences
	...
	var/list/bgstate_options = list("steel", "dark_steel", "wood", "white", "black")
...
/datum/category_item/player_setup_item/augmentation/content(var/mob/user)
	. = list()
	. += "<style>div.block{margin: 3px 0px;padding: 4px 0px;} span.box{display: inline-block; width: 20px; height: 10px; border:1px solid #000;}</style>"
...
//		. += "Skin Tone: <a href='?src=\ref[src];skin_tone=1'>[-pref.s_tone + 35]/[mob_species.max_skin_tone()]</a><br>"
	...
	*/
	...
	. += "<table style='max-height:400px;height:410px'>"
	. += "<tr style='vertical-align:top'><td><div style='max-width:230px;width:230px;height:100%;overflow-y:auto;border-right:1px solid;padding:3px'>"
	. += modifications_types[pref.current_organ]
	. += "</div></td><td style='margin-left:10px;width-max:285px;width:285px;'>"
	. += "<table><tr><td style='width:105px; text-align:right'>"
	...
	for(var/organ in pref.r_organs)
		...
		if(organ == pref.current_organ)
			. += "<div><b><span style='background-color:pink'>[organ_tag_to_name[organ]]</span></b> "
		else
			. += "<div><b>[organ_tag_to_name[organ]]</b> "
		if(mod.hascolor)
			. += "<a href='?src=\ref[src];color=[organ]'><span class='box' style='background-color:[pref.modifications_colors[organ]]'></span></a>"
		. += "<br><a href='?src=\ref[src];organ=[organ]'>[disp_name]</a></div>"
	...
	for(var/organ in pref.l_organs)
		...
		var/disp_name = mod ? mod.short_name : "Nothing"
		if(mod.hascolor)
			. += "<div><a href='?src=\ref[src];color=[organ]'><span class='box' style='background-color:[pref.modifications_colors[organ]]'></span></a>"
		if(organ == pref.current_organ)
			. += " <b><span style='background-color:pink'>[organ_tag_to_name[organ]]</span></b>"
		else
			. += " <b>[organ_tag_to_name[organ]]</b>"
		. += "<br><a href='?src=\ref[src];organ=[organ]'>[disp_name]</a></div>"
	...
	for(var/organ in pref.internal_organs)
		...
		if(organ == pref.current_organ)
			. += "<td width='33%'><b><span style='background-color:pink'>[organ_tag_to_name[organ]]</span></b>"
		else
			. += "<td width='33%'><b>[organ_tag_to_name[organ]]</b>"
		. += "<br><a href='?src=\ref[src];organ=[organ]'>[disp_name]</a></td>"
...
/datum/category_item/player_setup_item/augmentation/OnTopic(var/href, list/href_list, mob/user)
	...
	*/
		...
		var/list/valid_hairstyles = hair_styles_list//mob_species.get_hair_styles()
		...
		mob_species = all_species[pref.species]
		if(new_h_style && CanUseTopic(user) && (new_h_style in hair_styles_list/*mob_species.get_hair_styles()*/))
		...
		var/list/valid_facialhairstyles = facial_hair_styles_list//mob_species.get_facial_hair_styles(pref.gender)
		...
		//mob_species = all_species[pref.species]
		if(new_f_style && CanUseTopic(user) && facial_hair_styles_list/*mob_species.get_facial_hair_styles(pref.gender)*/)
			...
			if("Normal")
				...
				if(limb == BP_CHEST)
					for(var/other_limb in (BP_ALL_LIMBS - BP_CHEST))
						...
						for(var/internal_organ in list(O_HEART,O_EYES,O_LUNGS,O_LIVER,O_KIDNEYS,O_BRAIN))
...
/datum/category_item/player_setup_item/augmentation/proc/ResetAllHair()
	ResetHair()
	ResetFacialHair()

/datum/category_item/player_setup_item/augmentation/proc/ResetHair()
	/*
	var/datum/species/mob_species = all_species[pref.species]
...
/datum/category_item/player_setup_item/augmentation/proc/ResetFacialHair()
	/*
	var/datum/species/mob_species = all_species[pref.species]
...
/datum/category_item/player_setup_item/augmentation/proc/has_flag(var/datum/species/mob_species, var/flag)
