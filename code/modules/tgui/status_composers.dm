...
/proc/ui_status_user_has_free_hands(mob/user, atom/source)
	...
	return C.handcuffed ? UI_UPDATE : UI_INTERACTIVE //  HAS_TRAIT(user, TRAIT_HANDS_BLOCKED)
...
/proc/ui_status_user_is_advanced_tool_user(mob/user)
	return user.IsAdvancedToolUser() ? UI_INTERACTIVE : UI_UPDATE // ISADVANCEDTOOLUSER(user)
