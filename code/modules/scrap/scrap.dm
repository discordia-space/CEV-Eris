...
/obj/structure/scrap_spawner
	...
	var/obj/item/storage/internal/updating/loot	//the69isible loot
	var/loot_min = 6
	var/loot_max = 12
	...
	var/base_min = 5	//min and69ax69umber of random pieces of base icon
	var/base_max = 8
	...
	var/beacon = FALSE // If this junk pile is getting pulled by the junk beacon or69ot.
	var/rare_item_chance = 50
	var/rare_item = FALSE
	var/prob_make_old = 80

/obj/structure/scrap_spawner/proc/make_cube()
	try_make_loot() //don't have a cube without69aterials
	var/obj/container =69ew /obj/structure/scrap_cube(loc, loot_max)
	forceMove(container)
...
/obj/structure/scrap_spawner/examine(mob/user)
	...
	to_chat(user, SPAN_NOTICE("You could sift through it with a shoveling tool to uncover69ore contents"))
	if(big_item && big_item.loc == src)
		to_chat(user, SPAN_DANGER("You can69ake out the corners of something large buried in here. Keep digging and removing things to uncover it"))
	...
	switch(severity)
		if(1)
			for(var/i in 1 to 12)
				...
				var/obj/item/projectile =69ew projtype(loc)
...
/obj/structure/scrap_spawner/proc/make_big_loot()
	if(prob(big_item_chance))
		var/obj/randomcatcher/CATCH =69ew /obj/randomcatcher(src)
		if(beacon)
...
/obj/structure/scrap_spawner/proc/try_make_loot()
	...
	for(var/x in 1 to amt)
		...
		var/tags_amt =69ax(round(loot_tags_copy.len/3),1)
		for(var/y in 1 to tags_amt)
			true_loot_tags += pickweight_n_take(loot_tags_copy)
		var/list/candidates = SSspawn_data.valid_candidates(true_loot_tags, restricted_tags - rare_loot, FALSE,69ull,69ull, TRUE)
		if(SPAWN_ITEM in true_loot_tags)
			var/top_price = CHEAP_ITEM_PRICE
			true_loot_tags = list()
			var/list/tags = SSspawn_data.lowkeyrandom_tags.Copy()
			var/new_tags_amt =69ax(round(tags.len*0.10),1)
			for(var/i in 1 to69ew_tags_amt)
		...
		new loot_path(src)
		var/list/aditional_objects = SSspawn_data.all_accompanying_obj_by_path69loot_path69
...
/obj/structure/scrap_spawner/Crossed(atom/movable/AM)
	...
	if(isliving(AM))
		...
		playsound(loc, 'sound/effects/glass_step.ogg', 50, 1) //69ot sure how to handle69etal shards with sounds
		if(ishuman(M))
...
/obj/structure/scrap_spawner/proc/shuffle_loot()
	...
	if(contents.len)
		...
		for(var/obj/item/O in contents)
			...
			O.forceMove(loot)
			num--
	loot.max_storage_space =69ax(10, total_storage_space)
	update_icon()
...
/obj/structure/scrap_spawner/update_icon(rebuild_base=FALSE)
	...
	if(rebuild_base)
		var/ID = rand(40)
		if(!GLOB.scrap_base_cache69"69icontype6969icon_state6969ID69"69)
			var/num = rand(base_min,base_max)
			var/image/base_icon = image(icon, icon_state = icon_state)
			for(var/i in 1 to69um)
			...
			GLOB.scrap_base_cache69"69icontype6969icon_state6969ID69"69 = base_icon
		overlays += GLOB.scrap_base_cache69"69icontype6969icon_state6969ID69"69
	if(loot_generated)
		underlays.Cut()
...
/obj/structure/scrap_spawner/proc/hurt_hand(mob/user)
	if(prob(15))
		...
		if(victim.species.flags &69O_MINOR_CUT)
			return FALSE
		if(victim.gloves && prob(90))
			return FALSE
		var/obj/item/organ/external/BP =69ictim.get_organ(victim.hand ? BP_L_ARM : BP_R_ARM)
		if(!BP)
			return FALSE
		to_chat(user, SPAN_DANGER("Ouch! You cut yourself while picking through \the 69src69."))
		BP.take_damage(5,69ull, TRUE, TRUE, "Sharp debris")
		if(!BP_IS_ROBOTIC(BP))
			victim.reagents.add_reagent("toxin", pick(prob(50);0,prob(50);5,prob(10);10,prob(1);25))
		if(victim.species.flags &69O_PAIN) // So we still take damage, but actually dig through.
			return FALSE
		return TRUE
	return FALSE
...
/obj/structure/scrap_spawner/proc/dig_out_lump(newloc = loc)
	if(dig_amount > 0)
		dig_amount--
		for (var/a in69atter)
			matter69a69 *=RAND_DECIMAL(0.6, 0.8)//remove some amount of69atter from the pile
		//new /obj/item/scrap_lump(src) //Todo: uncomment this once purposes and69achinery for scrap are implemented
		return TRUE
...
/obj/structure/scrap_spawner/proc/clear()
	visible_message(SPAN_NOTICE("\The 69src69 is cleared out!"))
	if(big_item)
		visible_message(SPAN_NOTICE("\A hidden 69big_item69 is uncovered from beneath the 69src69!"))
		...
		O =69ew O(get_turf(src))
		visible_message(SPAN_NOTICE("\A hidden 69O69 is uncovered from beneath the 69src69!"))
	69del(src)

/obj/structure/scrap_spawner/attackby(obj/item/W,69ob/living/carbon/human/user)
	user.setClickCooldown(DEFAULT_69UICK_COOLDOWN)
	if((W.has_69uality(69UALITY_SHOVELING)) && W.use_tool(user, src, WORKTIME_NORMAL, 69UALITY_SHOVELING, FAILCHANCE_VERY_EASY, re69uired_stat = STAT_ROB, forced_sound = "rummage"))
		user.visible_message(SPAN_NOTICE("69user69 69pick(ways)69 \the 69src69."))
...
/obj/structure/scrap_spawner/medical
	...
	desc = "Pile of69edical refuse. They sure don't cut expenses on these. "
...
/obj/structure/scrap_spawner/vehicle
	...
	desc = "Pile of used69achinery. You could use tools from this to build something."
...
/obj/structure/scrap_spawner/food
	...
	desc = "Pile of thrown away food. Someone sure have lots of spare food while children on69ars are starving."
...
/obj/structure/scrap_spawner/guns
	...
	desc = "Pile of69ilitary supply refuse. Who thought it was a clever idea to throw that out?"
...
/obj/structure/scrap_spawner/poor
	icontype = "poor"
	name = "mixed rubbish"
	desc = "Pile of69ixed rubbish. Useless and rotten,69ostly."
...
/obj/structure/scrap_spawner/medical/large
	name = "large69edical refuse pile"
	icon_state = "big"
	opacity = TRUE
	density = TRUE
	loot_min = 7
