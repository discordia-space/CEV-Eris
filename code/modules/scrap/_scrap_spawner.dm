...
/obj/structure/scrap_spawner/proc/try_make_loot()
	...
	for(var/x in 1 to amt)
		...
		var/tags_amt = CLAMP(round(loot_tags.len*0.3),1, loot_tags.len)
		...
		var/top_spawn = candidates.len
		if(x >= amt-3 && prob(rare_item_chance))
			top_spawn = round(candidates.len * 0.5)
		candidates = lsd.only_top_candidates(candidates, top_spawn)
		var/loot_path = lsd.pick_spawn(candidates)
		new loot_path(src)
		var/aditional_object = lsd.all_accompanying_obj_by_path[loot_path]
		if(aditional_object && prob(25))
			new aditional_object(src)
...
/obj/structure/scrap_spawner/vehicle
	...
	loot_tags = list(
		...
		SPAWN_GUN_UPGRADE,
		SPAWN_POUCH,
		SPAWN_TAG_MATERIAL_RESOURCES_BULDING,
		SPAWN_JUNK, SPAWN_CLEANABLE,
		SPAWN_ORE,
		SPAWN_RARE_ITEM,
		SPAWN_TOOL_UPGRADE,
