/obj/item/rig/attackby(obj/item/I, mob/user)
	...
	switch(tool_type)
		...
		if(QUALITY_WELDING)
			...
			else if(I.use_tool(user, src, WORKTIME_SLOW, QUALITY_WELDING, FAILCHANCE_ZERO, required_stat = STAT_MEC))
				ablative_armor = min(ablative_armor + get_tool_quality(QUALITY_WELDING) / 2, ablative_max)
