	...
	for(var/obj/item/grab/G in grabbed_by)
		...
		switch(G.state)
			if(GRAB_PASSIVE)
				qdel(G)
			if(GRAB_AGGRESSIVE)
				if(prob(max(60 + ((stats?.getStat(STAT_ROB)) - (G.assailant?.stats.getStat(STAT_ROB)+0.001) ** 0.8), 1))) // same scaling as cooldown increase and if you manage to be THAT BAD, 1% for luck
