...
/mob/living/carbon/human/proc/handle_hud_list()
	...
	if (BITTEST(hud_updateflag, SPECIALROLE_HUD))
		...
		if(mind && mind.antagonist.len != 0)
			if(GLOB.hud_icon_reference[mind.antagonist[1].role_text]) //only display the first antagonist role
				holder.icon_state = GLOB.hud_icon_reference[mind.antagonist[1].role_text]
			else
