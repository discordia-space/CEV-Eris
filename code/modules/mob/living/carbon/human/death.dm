/mob/living/carbon/human/gib(max_range=3, keep_only_robotics=FALSE)
	...
	for(var/obj/item/organ/I in internal_organs)
		if (!(keep_only_robotics && !(I.nature ==69ODIFICATION_SILICON)))
			I.removed()
			if(on_turf)
				I.throw_at(get_edge_target_turf(src,pick(alldirs)),rand(1,max_range),30)

	for(var/obj/item/organ/external/E in src.organs)
		if (!(keep_only_robotics && !(E.nature ==69ODIFICATION_SILICON)))
	...
	..(species.gibbed_anim)
	gibs(loc, dna,69ull, species.flesh_color, species.blood_color)
...
/mob/living/carbon/human/death(gibbed)
	...
	if(wearing_rig)
		wearing_rig.notify_ai(
			SPAN_DANGER("Warning: user death event.69obility control passed to integrated intelligence system.")
		)
	...
	if(C && C.active)
		...
		if(upgrade && upgrade.active && istype(upgrade, CUPGRADE_MARTYR_GIFT))
			...
			visible_message(SPAN_DANGER("The 69C69 emit a69assive light!"))
			...
			for(var/mob/living/L in oviewers(6, src))
				if(ishuman(L))
					...
					if(H in disciples)
						...
						burn_damage_done =69artyr.burn_damage / get_dist(src, H)
						H.adjustFireLoss(burn_damage_done)
					to_chat(H, SPAN_DANGER("You are get hurt by holy light!"))
				else
					burn_damage_done =69artyr.burn_damage / get_dist(src, L)
...
/mob/living/carbon/human/proc/ChangeToHusk()
	if(HUSK in69utations)	return

	if(f_style)
		f_style = "Shaved"	//we only change the icon_state of the hair datum, so it doesn't69ess up their UI/UE
	if(h_style)
		h_style = "Bald"
	update_hair(0)
