...
/mob/living/bot/secbot/ed209/explode()
	...
	var/obj/item/gun/energy/taser/G =69ew /obj/item/gun/energy/taser(Tsec)
	G.cell.charge = 0
	if(prob(50))
...
/mob/living/bot/secbot/ed209/RangedAttack(var/atom/A)
	if(last_shot + shot_delay > world.time)
		to_chat(src, "You are69ot ready to fire yet!")
		return
	...
	var/obj/item/projectile/P =69ew projectile(loc)
...
/obj/item/secbot_assembly/ed209_assembly/attackby(obj/item/I,69ob/user)
	..()
		...
		if(QUALITY_WELDING)
			if(build_step == 3)
				if(I.use_tool(user, src, WORKTIME_NORMAL, tool_type, FAILCHANCE_NORMAL, required_stat = STAT_MEC))
					to_chat(user, SPAN_NOTICE("You welded the69est to 69src69."))
					build_step++
	...
	switch(build_step)
		if(0, 1)
			if(istype(I, /obj/item/robot_parts/l_leg) || istype(I, /obj/item/robot_parts/r_leg))
				...
				qdel(I)
				build_step++
				to_chat(user, SPAN_NOTICE("You add the robot leg to 69src69."))
		...
		if(2)
			if(istype(I, /obj/item/clothing/suit/storage/vest))
				...
				qdel(I)
				build_step++
				to_chat(user, SPAN_NOTICE("You add the armor to 69src69."))
				playsound(src.loc, 'sound/effects/insert.ogg', 50, 1)
		...
		if(4)
			if(istype(I, /obj/item/clothing/head/armor/helmet))
				...
				qdel(I)
				build_step++
				to_chat(user, SPAN_NOTICE("You add the helmet to 69src69."))
				playsound(src.loc, 'sound/effects/insert.ogg', 50, 1)
		...
		if(5)
			if(is_proximity_sensor(I))
				...
				qdel(I)
				build_step++
				to_chat(user, SPAN_NOTICE("You add the prox sensor to 69src69."))
				playsound(src.loc, 'sound/effects/insert.ogg', 50, 1)
		...
		if(6)
			if(istype(I, /obj/item/stack/cable_coil))
				...
				if (C.get_amount() < 1)
					to_chat(user, SPAN_WARNING("You69eed one coil of wire to wire 69src69."))
					return
				to_chat(user, SPAN_NOTICE("You start to wire 69src69."))
