...
/mob/proc/show_message(msg, type, alt, alt_type)//Message, type of message (1 or 2), alternative message, alt message type (1 or 2)
	...
	if(type)
		if(type & 1 && (sdisabilities & BLIND || blinded || hasStatusEffect(src, SE_PARALYZED))) //Vision related
			if(!alt)
				return
			else
				msg = alt
...
/mob/proc/Stun(amount)
	if(status_flags & CANSTUN)
		facing_dir = null
		addStatusEffect(src, SE_STUNNED, amount)
	return

/mob/proc/SetStunned(amount) //if you REALLY need to set stun to a set amount without the whole "can't go below current stunned"
	if(status_flags & CANSTUN)
		var/datum/statusEffect/effect = getStatusEffect(src, SE_PARALYZED)
...
/mob/proc/Paralyse(amount)
	if(status_flags & CANPARALYSE)
		facing_dir = null
		addStatusEffect(src, SE_PARALYZED, amount)
		return TRUE
	return FALSE
