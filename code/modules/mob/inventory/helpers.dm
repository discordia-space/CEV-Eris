...
/proc/mob_can_equip(mob/living/Mob, obj/item/Item, slot, disable_warning, force = FALSE)
	if(!istype(Mob) || !istype(Item) || !isnum(slot))
		return FALSE
	return Mob.can_equip(Item, slot, disable_warning, force) && Item.can_be_equipped(Mob, slot, disable_warning)


/mob/proc/can_equip(obj/item/Item, slot, disable_warning = FALSE, force = FALSE)
	if(!force && get_equipped_item(slot))
		if(!disable_warning)
...
/mob/living/carbon/human/can_equip(obj/item/Item, slot, disable_warning = FALSE, force = FALSE)
	if(!slot)
		return FALSE
	...
	if(S && !S.can_equip(Item, src, disable_warning))
		return FALSE
	return (force || slot_is_accessible(slot, Item, disable_warning? null : src)) && ..()
