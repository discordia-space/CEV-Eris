...
/obj/machinery/cooking_with_jane/oven/attack_hand(mob/user, params)
	if(getInput(params))
		if(!opened)
			handle_open(user)
		else
...
/obj/machinery/cooking_with_jane/oven/proc/handle_cooking(mob/user, set_timer)
	if(!(items && istype(items, /obj/item/reagent_containers/cooking_with_jane/cooking_container)))
		return
...
/obj/machinery/cooking_with_jane/oven/update_icon()
	...
	if(!opened)
		add_overlay(image(icon, icon_state="oven_hatch[switches?"_on":""]", layer = ABOVE_OBJ_LAYER))
