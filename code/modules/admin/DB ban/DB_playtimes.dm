		...
		for(var/job in SSinactivity.ckey_to_job_playtime[ckey])
			...
			if(!job_query.NextRow())
				...
				if(!insertion.Execute)
					message_admins("Failed to create a playtime database for [ckey] , [job] with error [insertion.ErrorMsg()]")
			else
...
/proc/whitelist_from_job_playtimes(ckey)
	...
	if(get_him.NextRow())
		...
		if(get_him.NextRow())
			...
			if(!insertion.Execute())
				message_admins("Failed to whitelist [ckey] due to error [insertion.ErrorMsg()]")
