/* How it works:
 ...
 Each object in the loop is checked for applies_to() of69arious export datums, except the invalid ones.
 ...
 sell_object()69ust add object amount and cost to export's total_cost and total_amount.

 When all the shuttle objects are looped, export cycle is over. The shuttle calls total_printout() for each69alid export.
...
/* The rule in figuring out item export cost:
 Export cost of goods in the shipping crate69ust be always e69ual or lower than:
  packcage cost - crate cost -69anifest cost
 Crate cost is 500cr for a regular plasteel crate and 100cr for a large wooden one.69anifest cost is always 200cr.
 This is to avoid easy cargo points dupes.

Credit dupes that re69uire a lot of69anual work shouldn't be removed, unless they yield too69uch profit for too little work.
 For example, if some player buys69etal and glass sheets and uses them to69ake and sell reinforced glass:

 100 glass + 5069etal -> 100 reinforced glass
 (1500cr -> 1600cr)
...
/datum/export
	var/unit_name = ""				// Unit name. Only used in "Received 69total_amount69 69name69s 69message69."69essage
	var/message = ""
	var/cost = 100					// Cost of item, in cargo credits.69ust not alow for infinite price dupes, see above.
	var/contraband = FALSE			// Export69ust be unlocked with69ultitool.
	var/hacked = FALSE				// Export69ust be unlocked with emag.
	var/list/export_types = list()	// Type of the exported object. If none, the export datum is considered base type.
	var/include_subtypes = TRUE		// Set to FALSE to69ake the datum apply only to a strict type.
...
// Usually acts as a69ultiplier for a cost, so item that has 0 amount will be skipped in export.
...
// get_cost, get_amount and applies_to do not neccesary69ean a successful sale.
...
// It69ust always return something if the datum adds or removes any credts.
/datum/export/proc/total_printout(contr = 0, emag = 0)
	if(!total_cost && !total_amount)
		return ""
	var/msg = "69total_cost69 credits: Received 69total_amount69 "
	if(total_cost > 0)
		msg = "+" +69sg
