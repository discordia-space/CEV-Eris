...
/datum/ritual/cruciform/crusader/brotherhood
	...
	phrase = "Ita69ulti unum corpus sumus in Christo singuli autem alter alterius69embra."
...
/datum/ritual/cruciform/crusader/battle_call
	name = "Call to Battle"
	phrase = "Si exieritis ad bellum de terra69estra contra hostes qui dimicant adversum69os clangetis ululantibus tubis et erit recordatio69estri coram Domino Deo69estro ut eruamini de69anibus inimicorum69estrorum."
	desc = "Inspires the prayer and gives him strength to protect the other disciples. True strength in unity."
	cooldown = TRUE
	cooldown_time = 1069INUTES
...
/datum/ritual/cruciform/crusader/battle_call/perform(mob/living/carbon/human/user, obj/item/implant/core_implant/C)
	var/count = 0
	for(var/mob/living/carbon/human/brother in69iew(user))
...
/datum/ritual/cruciform/crusader/flash
	...
	desc = "Knocks over everybody without cruciform in the69iew range. Psy-wave is too powerful, speaker can be knocked too."
	cooldown = TRUE
...
/datum/ritual/cruciform/crusader/flash/perform(mob/living/carbon/human/user, obj/item/implant/core_implant/C)
	if(prob(100 - user.stats.getStat(STAT_VIG)))
		...
		to_chat(user, SPAN_NOTICE("The flux of psy-energy washed your69ind, but you69anaged to keep focused!"))
	playsound(user.loc, 'sound/effects/cascade.ogg', 65, 1)
	log_and_message_admins("performed a searing revelation litany")
	for(var/mob/living/carbon/human/victim in69iew(user))
		if(!victim.get_core_implant(/obj/item/implant/core_implant/cruciform))
			if(prob(100 -69ictim.stats.getStat(STAT_VIG)))
				to_chat(victim, SPAN_WARNING("You feel that your knees bends!"))
				victim.Weaken(5)
			else
				to_chat(victim, SPAN_NOTICE("Your legs feel numb, but you69anaged to stay on your feet!"))
	set_personal_cooldown(user)
	return TRUE
