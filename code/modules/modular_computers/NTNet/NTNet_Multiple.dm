var/global/list/datum/ntnet/nt_networks = list()
var/global/nt_counter = 0


// This is the NTNet datum. There can be only one NTNet datum in game at once. Modular computers read data from this.
/datum/ntnet/
	// can have multiple relays so it links to multiple divided z-levels
	...
	// used to link relays togheter and identification
	var/network_id
...
/datum/ntnet/proc/get_computer_by_nid(var/NID)
	for(var/obj/item/modular_computer/comp in SSobj.processing)
		if(comp && comp.network_card && && comp.network_id == network_id && comp.network_card.identification_id == NID)
			return comp
...
/datum/ntnet/proc/sort_email_list()
	...
	if(email_accounts.len > 1)
		for(var/i = 1, i <= email_accounts.len, i++)
			var/flag = FALSE
			for(var/j = 1, j <= email_accounts.len - 1, j++)
				var/datum/computer_file/data/email_account/EA = email_accounts[j]
				var/datum/computer_file/data/email_account/EA_NEXT = email_accounts[j+1]
				if(sorttext(EA.ownerName, EA_NEXT.ownerName) == -1)
					flag = TRUE
					email_accounts.Swap(j,j+1)
			if(!flag)
				break
...
proc/ntNetworkById(var/id)
	for(var/datum/ntnet/network in nt_networks)
		if(network.network_id == id)
			return network
	return FALSE
