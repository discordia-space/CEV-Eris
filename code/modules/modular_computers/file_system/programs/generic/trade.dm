...
/datum/computer_file/program/trade/Topic(href, href_list)
	...
	if(account)
		...
		if(href_list["PRG_offer_fulfill"])
			...
			log_admin("Station: [S.name]. Href: [href_list["PRG_offer_fulfill"]]. Href text2num: [text2num(href_list["PRG_offer_fulfill"])]")
			if(!S)
				return
			var/atom/movable/path = text2path(href_list["PRG_offer_fulfill_path"])
			log_admin("Path: [path]. Is type: [istype(path)]. Is path: [ispath(path)].")
...
/datum/nano_module/program/trade/ui_data()
	...
	.["offers"] = list()
	for(var/datum/trade_station/S in SStrade.discovered_stations)
		for(var/offer_path in S.special_offers)
			var/atom/movable/path = offer_path
			var/list/offer_content = S.special_offers[offer_path]
			var/list/offer = list("station" = S.name, "name" = offer_content["name"], "amount" = offer_content["amount"], "price" = offer_content["price"], "index" = SStrade.discovered_stations.Find(S), "path" = path)
			if(PRG.sending)
				offer["available"] = length(SStrade.assess_offer(PRG.sending, S, offer_path))
			.["offers"] += list(offer)
