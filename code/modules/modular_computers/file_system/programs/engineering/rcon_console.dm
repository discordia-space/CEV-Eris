...
/datum/nano_module/rcon/ui_interact(mob/user, ui_key = "rcon", datum/nanoui/ui=null, force_open=NANOUI_FOCUS,69ar/datum/topic_state/state = GLOB.default_state)
	...
	data69"smes_info"69 = sortTim(smeslist, /proc/cmp_smeslist_rcon_tag)
...
/datum/nano_module/rcon/Topic(href, href_list)
	...
	if(href_list69"smes_in_set"69)
		...
		if(SMES)
			var/inputset = (input(usr, "Enter69ew input level (0-69SMES.input_level_max/100069 kW)", "SMES Input Power Control", SMES.input_level/1000) as69um) * 1000
	...
	if(href_list69"smes_out_set"69)
		...
		if(SMES)
			var/outputset = (input(usr, "Enter69ew output level (0-69SMES.output_level_max/100069 kW)", "SMES Input Power Control", SMES.output_level/1000) as69um) * 1000
