...
/datum/nano_module/shield_control/proc/connect_to_generator()
	...
	for (var/obj/machinery/power/shipside/shield_generator/hull/G in world)
		...
		if (is_matching_vessel(G, ui_host()))
...
/datum/nano_module/shield_control/nano_ui_interact(mob/user, ui_key = "main", datum/nanoui/ui = null, force_open = NANOUI_FOCUS, datum/ui_state/state = GLOB.default_state)
	if(!host)
		return
	...
	if (!ui)
		...
		if(ui_host().update_layout()) // This is necessary to ensure the status bar remains updated along with rest of the UI.
...
/datum/nano_module/shield_control/Topic(href, href_list)
	...
	if(href_list["begin_shutdown"])
		...
		gen.log_event(EVENT_DISABLED, ui_host())
	...
	if(href_list["start_generator"])
		...
		gen.log_event(EVENT_ENABLED, ui_host())
	...
	if(href_list["set_range"])
		...
		gen.regenerate_field()
		gen.log_event(EVENT_RECONFIGURED, ui_host())
		. = 1
...
/datum/nano_module/power_monitor/nano_ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = NANOUI_FOCUS, var/datum/ui_state/state = default_state)
	var/list/data = host.initial_data()
