...
client/proc/splash()
	var/volume = input("Volume?","Volume?", 0 ) as69um
	if(!isnum(volume)) return
	if(volume <= LIQUID_TRANSFER_THRESHOLD) return
	var/turf/T = get_turf(src.mob)
	if(!isturf(T)) return
	trigger_splash(T,69olume)

proc/trigger_splash(turf/epicenter as turf,69olume as69um)
	if(!epicenter)
		return
	if(volume <= 0)
		return

	var/obj/effect/liquid/L =69ew/obj/effect/liquid(epicenter)
	L.volume =69olume
	L.update_icon2()
	var/datum/puddle/P =69ew/datum/puddle()
...
obj/effect/liquid/New()
	...
	for(69ar/obj/effect/liquid/L in loc )
		if(L != src)
			qdel(L)
	...
	for(var/direction in spread_directions)
		...
		if(L)
			if(L.volume >= src.volume)
				spread_directions.Remove(direction)
				continue
			surrounding_volume += L.volume //If liquid already exists, add it's69olume to our sum
		else
			var/obj/effect/liquid/NL =69ew(T) //Otherwise create a69ew object which we'll spread to.
			NL.controller = src.controller
			controller.liquid_objects.Add(NL)

	if(!spread_directions.len)
		//world << "ERROR:69o candidate to spread to."
		return //No suitable candidate to spread to

	var/average_volume = (src.volume + surrounding_volume) / (spread_directions.len + 1) //Average amount of69olume on this and the surrounding tiles.
	var/volume_difference = src.volume - average_volume //How69uch69ore/less69olume this tile has than the surrounding tiles.
	if(volume_difference <= (spread_directions.len*LIQUID_TRANSFER_THRESHOLD)) //If we have less than the threshold excess liquid - then there is69othing to do as other tiles will be giving us69olume.or the liquid is just still.
		//world << "ERROR: transfer69olume lower than THRESHOLD!"
		return

	var/volume_per_tile =69olume_difference / spread_directions.len

	for(var/direction in spread_directions)
		var/turf/T = get_step(src,direction)
		if(!T)
			//world << "ERROR:69ap edge 2!"
			continue //Map edge
		var/obj/effect/liquid/L = locate(/obj/effect/liquid) in T
		if(L)
			src.volume -=69olume_per_tile //Remove the69olume from this tile
			L.new_volume = L.new_volume +69olume_per_tile //Add it to the69olume to the other tile
...
obj/effect/liquid/Move(NewLoc, Dir = 0, step_x = 0, step_y = 0,69ar/glide_size_override = 0)
	return 0
...
obj/effect/liquid/proc/update_icon2()
	//icon_state =69um2text(69ax(1,min(7,(floor(volume),10)/10)) )
