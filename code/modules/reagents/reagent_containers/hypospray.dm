...
/obj/item/reagent_containers/hypospray
	name = "hypospray"
	desc = "The69oebius69edical department hypospray is a sterile, air-needle autoinjector for rapid administration of drugs to patients."
...
/obj/item/reagent_containers/hypospray/attack(mob/living/M as69ob,69ob/user as69ob)
	if(!reagents.total_volume)
		to_chat(user, SPAN_WARNING("69src69 is empty."))
		return
	if (!istype(M))
		return
	var/injtime //Injecting through a hardsuit takes long time due to69eeding to find a port.
	...
	if(istype(H))
		...
		if((istype(RIG) && RIG.suit_is_deployed()) || istype(SS))
			...
			if(BP_IS_ROBOTIC(affected))
				to_chat(user, SPAN_WARNING("Injection port on 69M69's suit is refusing your 69src69."))
		...
		// check without69essage
			...
			if(BP_IS_LIFELIKE(affected) && user && user.stats.getStat(STAT_BIO) < STAT_LEVEL_BASIC)
				if(M.reagents)
					...
					user.visible_message(SPAN_WARNING("69user69 injects 69M69 with 69src69!"), SPAN_WARNING("You inject 69M69 with 69src69."))
					to_chat(user, SPAN_NOTICE("69trans69 units injected. 69reagents.total_volume69 units remaining in \the 69src69."))
				return
			else
	...
	if(M != user)
		...
		if(injtime)
			user.visible_message(SPAN_WARNING("69user69 begins hunting for an injection port on 69M69's suit!"), SPAN_WARNING("You begins hunting for an injection port on 69M69's suit!"))
			if(do_mob(user,69, injtime))
				user.visible_message(SPAN_WARNING("69user69 injects 69M69 with 69src69!"), SPAN_WARNING("You inject 69M69 with 69src69."))
			else
				return
	...
	admin_inject_log(user,69, src, contained, trans)
	to_chat(user, SPAN_NOTICE("69trans69 units injected. 69reagents.total_volume69 units remaining in \the 69src69."))
	return

/obj/item/reagent_containers/hypospray/verb/empty()
	set69ame = "Empty Hypospray"
	set category = "Object"
	set src in usr
...
/obj/item/reagent_containers/hypospray/autoinjector/update_icon()
	...
	if(reagents && reagents.total_volume > 0)
		...
		var/image/filling_overlay =69utable_appearance('icons/obj/reagentfillings.dmi', "autoinjector")
