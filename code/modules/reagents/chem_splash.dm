...
// Threatscale is a69ultiplier for the 'threat' of the grenade. If you're increasing the affected range drastically, you69ight want to improve this.
// Extra heat affects the temperature of the69ixture, and69ay cause it to react in different ways.
...
/proc/chem_splash(turf/epicenter, affected_range = 3, list/datum/reagents/reactants = list(), extra_heat = 0, threatscale = 1)
	...
	var/datum/reagents/splash_holder =69ew/datum/reagents(total_reagents*threatscale)
	splash_holder.my_atom = epicenter // For some reason this is setting69y_atom to69ull, and causing runtime errors.
	...
	splash_holder.handle_reactions() // React them69ow.

	if(splash_holder.total_volume)	//The possible reactions didnt use up all reagents, so we spread it around.
		var/datum/effect/effect/system/steam_spread/steam =69ew /datum/effect/effect/system/steam_spread()
		steam.set_up(10, 0, epicenter)
		steam.attach(epicenter)
		steam.start()

		var/list/viewable =69iew(affected_range, epicenter)
		...
		for(var/i=1; i<=affected_range; i++)
			...
			for(var/turf/T in turflist)
				if(!(get_dir(T,epicenter) in cardinal) && (abs(T.x - epicenter.x) == abs(T.y - epicenter.y) ))
					turflist.Remove(T)
					turflist.Add(T) // we69ove the purely diagonal turfs to the end of the list.
			for(var/turf/T in turflist)
				if(accessible69T69)
					continue
		...
		if(!reactable.len) //Nothing to react with. Probably69eans we're in69ullspace.
			return
		for(var/thing in reactable)
			var/atom/A = thing
			splash_holder.touch(A)

			//var/distance =69ax(1,get_dist(A, epicenter))
