...
/datum/chemical_reaction
	...
	var/requireElectrolysis = FALSE

	// Require strong gravitational pull ? (Centrifuge)
	var/requireStrongGravity = FALSE

	//Limits progress per reaction (keep in mind that if on reaction end it will repeat itself if all reqirements are met, this is intended for reactions that can change states of reagents like temperature or electrolisis state)
	//var/progressPerReaction = INFINITY
...
/datum/chemical_reaction/proc/can_happen(datum/reagents/holder)
	...
	if(requireStrongGravity && !holder.affectedByStrongGravity())
		return FALSE
...
/datum/chemical_reaction/proc/process(datum/reagents/holder)
	...
	//var/reaction_progress = min(reaction_limit, progress_limit, progressPerReaction) //no matter what, the reaction progress cannot exceed the stoichiometric limit.
