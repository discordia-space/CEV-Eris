...
/obj/item/device/makeshift_chem_master/New()
	. = ..()
	if(!reagents)
		create_reagents(60)

/obj/item/device/makeshift_chem_master/Destroy()
	QDEL_NULL(beaker)
	return ..()

/obj/item/device/makeshift_chem_master/AltClick(mob/living/user)
...
/obj/item/device/makeshift_chem_master/MouseDrop_T(atom/movable/I, mob/user, src_location, over_location, src_control, over_control, params)
	if(!Adjacent(user) || !I.Adjacent(user) || user.stat)
		return ..()
	if(istype(I, /obj/item/reagent_containers) && I.is_open_container() && !beaker)
		I.forceMove(src)
		...
		to_chat(user, SPAN_NOTICE("You add [I] to [src]."))
		updateUsrDialog()
		icon_state = "mixer_makeshift_occupied"
		return
	. = ..()

/obj/item/device/makeshift_chem_master/attackby(var/obj/item/B as obj, var/mob/user as mob)
	if(istype(B, /obj/item/reagent_containers/glass))
...
/obj/item/device/makeshift_chem_master/attack_self()
	mixer_interact(usr)

/obj/item/device/makeshift_chem_master/Topic(href, href_list)
	if(..())
		return 1
	...
	if(beaker)
		...
		if (href_list["analyze"])
			...
			if(href_list["name"] == "Blood")
				...
				var/B = G.data["blood_type"]
				var/C = G.data["blood_DNA"]
				dat += "<TITLE>Chemmaster 2000</TITLE>Chemical infos:<BR><BR>Name:<BR>[A]<BR><BR>Description:<BR>Blood Type: [B]<br>DNA: [C]<BR><BR><BR><A href='?src=\ref[src];main=1'>(Back)</A>"
			else
				dat += "<TITLE>Chemmaster 2000</TITLE>Chemical infos:<BR><BR>Name:<BR>[href_list["name"]]<BR><BR>Description:<BR>[href_list["desc"]]<BR><BR><BR><A href='?src=\ref[src];main=1'>(Back)</A>"
			usr << browse(dat, "window=chem_master;size=575x400")
			return
...
/obj/item/device/makeshift_chem_master/proc/mixer_interact(mob/user)
	if(!(usr.client in has_sprites))
		spawn()
			has_sprites += user.client
			for(var/i = 1 to MAX_PILL_SPRITE)
				usr << browse_rsc(icon('icons/obj/chemical.dmi', "pill" + num2text(i)), "pill[i].png")
	...
	if(!beaker)
		...
		dat += "<HR><BR><A href='?src=\ref[src];createpill=1'>Create pill ([max_pill_count] units max)</A><a href=\"?src=\ref[src]&change_pill=1\"><img src=\"pill[pillsprite].png\" /></a><BR>"
		dat += "<A href='?src=\ref[src];createpill_multiple=1'>Create multiple pills</A><BR>"
	user << browse("<TITLE>Chemmaster 2000</TITLE>Chemmaster menu:<BR><BR>[dat]", "window=chem_master;size=575x400")
	onclose(user, "chem_master")
	return
