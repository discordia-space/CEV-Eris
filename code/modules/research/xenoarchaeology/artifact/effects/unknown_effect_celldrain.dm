	...
	if(user)
		if(isrobot(user))
			...
			for (var/obj/item/cell/large/D in R.contents)
				D.charge =69ax(D.charge - rand() * 100, 0)
...
/datum/artifact_effect/celldrain/DoEffectPulse()
	if(holder)
		...
		for (var/obj/machinery/power/apc/C in range(200, T))
			for (var/obj/item/cell/large/B in C.contents)
				B.charge =69ax(B.charge - rand() * 150,0)
		for (var/obj/machinery/power/smes/S in range (src.effectrange,src))
			S.charge =69ax(S.charge - 250,0)
		for (var/mob/living/silicon/robot/M in range(100, T))
			for (var/obj/item/cell/large/D in69.contents)
				D.charge =69ax(D.charge - rand() * 150,0)
				if(world.time - last_message > 200)
					to_chat(M, "\red SYSTEM ALERT: Energy drain detected!")
					last_message = world.time
	return 1
