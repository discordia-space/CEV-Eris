...
HUDoverlays - Список "технических" элементов. С помощью данного списка заполняются HUDtech у моба
...
"icon" - используемый dmi файл, перезаписывает icon самого датума, необязателен
"state" - используемая иконка, необязательный параметр, для перезаписи данных у самого ХУДа
...
"r_hand" - имя Слота
"loc" - положение на экране
"name" - название элемента худа
"state" - какая иконка будет использована
"dir" - положение, дополнительный параметр
"toggle" = 1 - доп параметр, используется для функции по скорытию элементов ХУДа
...
Используемые уникальные переменные
	...
	var/process_flag = 0 - флаг необходимости вызова функции process()
...
________________________________________________________________________________________________________________________
Используемые функции на уровне моба
/mob/proc/create_HUD() - Функция, для создания элементов ХУДа
/mob/living/proc/destroy_HUD() - Функция, для уничтожения элементов ХУДа
/mob/living/proc/show_HUD() - Функция, для отображения элементов ХУДа у клиента

/mob/living/proc/check_HUD() - Основная функция, в ней анализируется "правильность" ХУДа, и через неё идет создание\вывод элементов на экран
...
Следующие функции создают элементы в соотвествующих массивов
...
2) При создании новой функции для работы с ситемой, пишите её "название" и приписывайте в конце _HUD. Пример: create_HUD(), show_HUD().
3) При переписывания кода под особенности подтипа /obj/screen, создайте новый файл с названием "[species_name]_screen_object",
если вы создаете элементы для "расы", если пишите код для нового ХУДа, то создайте файл с названием "[species_name]_[HUDname]_screen_object".
