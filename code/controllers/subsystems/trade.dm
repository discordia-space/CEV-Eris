...
SUBSYSTEM_DEF(trade)
	...
	var/trade_stations_budget = 40 // The map is 200x200 and Nostromo13 is focused on space exploration. We should be able to find new stations as we explore.
...
/datum/controller/subsystem/trade/proc/get_sell_price(path, datum/trade_station/trade_station)		// Janky proc, but I don't want to fuck with anything that's already using get_export_cost() -BHL
...
/datum/controller/subsystem/trade/proc/fulfill_offer(obj/machinery/trade_beacon/sending/beacon, datum/money_account/account, datum/trade_station/station, atom/movable/offer_path)
	...
	log_admin("Station: [station.name]. Offer: [offer_path]. Price: [offer_price]. Amount: [offer_amount].")
