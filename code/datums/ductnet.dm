...
/datum/ductnet
	...
	var/capacity = MACHINE_REAGENT_TRANSFER

/datum/ductnet/proc/update_capacity(all_check = TRUE, new_capacity=MACHINE_REAGENT_TRANSFER)
	if(!all_check && new_capacity < capacity)
		capacity = new_capacity
		return
	else if(all_check)
		capacity = MACHINE_REAGENT_TRANSFER
		for(var/obj/machinery/duct/D in ducts)
...
/datum/ductnet/proc/add_duct(obj/machinery/duct/D)
	...
	ducts += D
	D.duct = src
	update_capacity(FALSE, D.capacity)
