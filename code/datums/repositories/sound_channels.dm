...
/repository/sound_channels/proc/request(key)
	if(!key)
...
/repository/sound_channels/proc/release(channel)
...
/repository/sound_channels/proc/release_by_key(key)
...
/repository/sound_channels/proc/get_by_key(key)
	var/channel = LAZYACCESS(channels_by_key, "[key]")
	return channel
