...
/datum/component/gps/Initialize(_gpstag = "COM", _hide_prefixes = list("PDA", "TAB", "MPC", "IMP", "SPY"))
	if(!isatom(parent))
		return COMPONENT_INCOMPATIBLE
	gpstag = _gpstag
	hide_prefixes = _hide_prefixes
	serial_number = random_id("gps_id",1000,9999)
	GLOB.GPS_list += src
...
/datum/component/gps/item/Initialize(_gpstag = "COM", _hide_prefixes = list("PDA", "TAB", "MPC", "IMP", "SPY"), emp_proof = FALSE, state = null)
	. = ..()
	...
	A.name = "[initial(A.name)] ([gpstag]-[serial_number])"
...
/datum/component/gps/item/proc/on_emp_act(datum/source, severity)
	...
	var/atom/A = parent
	A.cut_overlays()
	A.add_overlays(image(A.icon, "emp"))
...
/datum/component/gps/item/proc/toggletracking(mob/user)
	...
	if(tracking)
		...
		A.add_overlays(image(A.icon, "working"))
...
/datum/component/gps/item/ui_data(mob/user)
	...
	for(var/gps in GLOB.GPS_list)
		...
		if(G.emped || !G.tracking || G == src || (G.gpstag in hide_prefixes))
			continue
		var/turf/pos = get_turf(G.parent)
		if(!pos || !global_mode && pos.z != curr.z)
			continue
		var/list/signal = list()
		signal["entrytag"] = "[G.gpstag]-[G.serial_number]" //Name or 'tag' of the GPS
