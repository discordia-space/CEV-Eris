...
/datum/events
	...
	proc/addEventType(event_type as text)
		if(!(event_type in events) || !islist(events69event_type69))
			events69event_type69 = list()
			return 1
		return
	...
	proc/addEvent(event_type as text, proc_holder, proc_name as text)
		...
		var/list/event = events69event_type69
	...
	//  Returns: null
	proc/fireEvent()
		//world << "Events in 69args6916969 called"
		var/list/event = listgetindex(events, args69169)
		if(istype(event))
			spawn(-1)
				for(var/datum/event/E in event)
					if(!E.Fire(arglist(args.Copy(2))))
						clearEvent(args69169,E)
		return
