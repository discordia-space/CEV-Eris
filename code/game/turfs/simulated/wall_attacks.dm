	...
	switch(tool_type)
		...
		if(QUALITY_WELDING)
			if(locate(/obj/effect/overlay/wallrot) in src)
				if(I.use_tool(user, src, WORKTIME_FAST, QUALITY_WELDING, FAILCHANCE_VERY_EASY))
		...
		if(QUALITY_PRYING)
			if(construction_stage == 3)
				...
				if(I.use_tool(user, src, WORKTIME_FAST, QUALITY_PRYING, FAILCHANCE_VERY_EASY))
		...
		if(QUALITY_CUTTING)
			if(construction_stage == 6)
				...
				if(I.use_tool(user, src, WORKTIME_FAST, QUALITY_CUTTING, FAILCHANCE_VERY_EASY))
		...
		if(QUALITY_SCREW_DRIVING)
			if(construction_stage == 5)
				...
				if(I.use_tool(user, src, WORKTIME_FAST, QUALITY_SCREW_DRIVING, FAILCHANCE_VERY_EASY))
