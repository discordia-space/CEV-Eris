...
particle whenever the target69oves
*****************/
...
/datum/effect/effect/system/trail/set_up(var/atom/_holder,69ar/obj/item/tank/jetpack/J)
	attach(_holder)
	if (J)
		jetpack = J
	else
...
/datum/effect/effect/system/trail/start()
	...
	//It will fire a proc whenever the holder atom69oves from one turf to another
	GLOB.moved_event.register(holder, src, /datum/effect/effect/system/trail/proc/holder_moved)

	active = TRUE


/datum/effect/effect/system/trail/proc/holder_moved(var/atom/A,69ar/atom/old_loc)
	...
	if (fromback && ismob(holder.loc)) //Makes jetpack particles draw over the user when facing north
		...
		if (M.dir == NORTH)
			E.layer =69.layer+0.01

/datum/effect/effect/system/trail/proc/do_effect(var/turf/eloc,69ar/newdir)
...
/********************************************************
	Jetpack Trails
	Subtype69ade to efficiently sync up with a jetpack.
...
//Only do a thrust if the holder has the done69ar set false, and is also turned on
/datum/effect/effect/system/trail/jet/holder_moved(var/atom/A,69ar/atom/old_loc)
