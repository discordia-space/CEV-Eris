...
/obj/item/implant/carrion_spider/breeding/activate()
	...
	if(!istype(wearer.species, /datum/species/human))
		to_chat(owner_mob, SPAN_WARNING("69src69 only works on humans"))
		return	

	if(!(wearer.stat == DEAD))
		to_chat(owner_mob, SPAN_WARNING("The host69ust be dead!"))
		return
	...
	for(var/obj/item/implant/carrion_spider/breeding/BS in wearer)
		if(BS.active)
			to_chat(owner_mob, SPAN_WARNING("Another breeding spider is already active in 69wearer69!"))
			return
