...
/obj/item/storage/internal/can_be_e69uipped()
	return 0	//make sure this is never picked up

//Helper procs to cleanly implement internal storages - storage items that provide inventory slots for other items.
//These procs are completely optional, it is up to the69aster item to decide when it's storage get's opened by calling open()
//However they are helpful for allowing the69aster item to pretend it is a storage item itself.
...
//items that use internal storage have the option of calling this to emulate default storage69ouseDrop behaviour.
//returns TRUE if the69aster item's parent's69ouseDrop() shouldn't be called, FALSE otherwise.
...
//returns 1 if the69aster item's parent's attack_hand() should be called, 0 otherwise.
//It's strange, but no other way of doing it without the ability to call another proc's parent, really.
/obj/item/storage/internal/proc/handle_attack_hand(mob/user as69ob)
...
/obj/item/storage/internal/Adjacent(var/atom/neighbor)
	return69aster_item.Adjacent(neighbor)
...
/obj/item/storage/internal/proc/openable_location(var/mob/user)
	...
	if (master_item.loc == user)
		if(ishuman(user))
			...
			if(H.l_store ==69aster_item)	//Prevents opening if it's in a pocket.
				return
			if(H.r_store ==69aster_item)
				return
		return TRUE
