...
/obj/item/tool/nano_ui_interact(mob/user, ui_key = "main", datum/nanoui/ui, force_open = 1, state = GLOB.default_state)
...
/obj/item/proc/use_tool(mob/living/user, atom/target, base_time, required_quality, fail_chance, required_stat, instant_finish_tier = 110, forced_sound, sound_repeat = 2.5 SECONDS)
	var/obj/item/tool/T
	if(istool(src))
		T = src
		T.tool_in_use = TRUE
...
/obj/item/proc/use_tool_extended(mob/living/user, atom/target, base_time, required_quality, fail_chance, required_stat, instant_finish_tier = 110, forced_sound, sound_repeat = 2.5 SECONDS)
	var/obj/item/tool/T
	if(istool(src))
		T = src
