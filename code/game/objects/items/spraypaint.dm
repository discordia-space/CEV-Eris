...
/obj/item/spraypaint/afterattack(atom/target, mob/user as mob, proximity)
	...
	if(istype(target,/turf))
		var/graffititype = input("Choose what you'd like to paint") in list("Kot","Onestar","Excelsior","Clown","Skull","Heart","Piss","Doodle","Ironhammer","Moebius","Aster","Ancap Yes","Ancap No","Neotheology","Technomancer","Eye")
		if(!Adjacent(target))
			to_chat(user, SPAN_NOTICE("You are too far away to paint anything."))
			return FALSE
		if(uses <= 0)
