/obj/item/device/techno_tribalism
	...
	description_info = "Has a internal radio that informs technomancers of its delay, it can be reenabled with a screwdriver if its not functioning. Deconstructing other departamental oddities reduces its cooldown"
...
/obj/item/device/techno_tribalism/attackby(obj/item/W, mob/user, params)
	if(nt_sword_attack(W, user))
		return FALSE
	if(user.a_intent == I_HELP && W.get_tool_quality(QUALITY_SCREWDRIVING))
