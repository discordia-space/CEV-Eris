...
/obj/item/device/radio/nano_ui_interact(mob/user, ui_key = "main", datum/nanoui/ui, force_open = NANOUI_FOCUS)
	var/data[0]
...
/obj/item/device/radio/talk_into(mob/living/M, message, channel, verb = "says", datum/language/speaking, speech_volume)
	if(!on)
	...
	*/
	...
	if(ishuman(M)) // --- Human: use their actual job ---
...
/obj/item/device/radio/borg/talk_into(mob/living/M, message, channel, verb = "says", datum/language/speaking, speech_volume)
	. = ..()
	if(isrobot(loc))
...
/obj/item/device/radio/borg/nano_ui_interact(mob/user, ui_key = "main", datum/nanoui/ui, force_open = NANOUI_FOCUS)
	var/data[0]
	data["mic_status"] = broadcasting
	data["speaker"] = listening
	data["freq"] = format_frequency(frequency)
