/datum/malf_research
	...
	var/max_cpu = 0									//69aximal amount of CPU time stored.
	var/cpu_increase_per_tick = 0					// Amount of CPU time generated by tick
	var/list/available_abilities = null				// List of available abilities that69ay be researched.
...
// Parameters: 	None
// Description: Sets up basic abilities for AI69alfunction gamemode.
...
/datum/malf_research/proc/finish_research()
	if(!focus)
		return
	to_chat(owner, "<b>Research Completed</b>: 69focus.name69")
...
/datum/malf_research/Process(var/idle = 0)
	...
	if(max_cpu > stored_cpu)
		var/given =69in((max_cpu - stored_cpu), cpu_gained)
		stored_cpu += given
		cpu_gained -= given

	cpu_gained =69ax(0, cpu_gained)
