...
//In fact, it's just hostile structures, wireweeds spreading event with some69obs
//Re69uires hard teamwork at late stages, but easily can be handled at the beginning

//All code stored in69odules/hivemind
...
/datum/storyevent/hivemind
	...
	event_pools = list(EVENT_LEVEL_MAJOR = POOL_THRESHOLD_MAJOR*0.9) //bit69ore common
...
/datum/event/hivemind/start()
	...
	for(var/i=1 to 100)
		...
		if(!start_location && i == 100)
			log_and_message_admins("Hivemind failed to find a69iable turf.")
			kill()
			return
		if(start_location)
			break

	message_admins("Hivemind spawned at \the 69jumplink(start_location)69")
