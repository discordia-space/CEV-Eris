...
/////////////////////////////////////////FIRST RUNE
	proc
		...
		//	var/tempnum = 0
			...
			if(index >= 5)
				user << SPAN_DANGER("You feel pain, as rune disappears in reality shift caused by too much wear of space-time fabric.")
				if (isliving(user))
		...
		convert()
			...
			if(!target) //didn't find any new targets
				if(!converting.len)
					...
					usr << SPAN_DANGER("You sense that the power of the dark one is already working away at them.")
				return
			...
			while(target in converting)
				...
				if (target.species && (target.species.flags & NO_PAIN))
					...
					target.visible_message(SPAN_WARNING("[target] writhes in pain as the markings below \him glow a bloody red."), SPAN_DANGER("AAAAAAHHHH!"), SPAN_WARNING("You hear an anguished scream."))
				...
				if(!waiting_for_input[target]) //so we don't spam them with dialogs if they hesitate
					...
					if(!cult.can_become_antag(target.mind) || jobban_isbanned(target, "cultist"))//putting jobban check here because is_convertable uses mind as argument
						...
						target << SPAN_DANGER("And you were able to force it out of your mind. You now know the truth, there's something horrible out there, stop it and its minions at all costs.")
				...
				if(R.word1==cultwords["travel"] && R.word2==cultwords["blood"] && R.word3==cultwords["self"])
					for(var/mob/living/carbon/D in R.loc)
						if(D.stat!=2)
							...
							D << SPAN_WARNING("You feel weakened.")
		...
		seer()
			if(usr.loc==src.loc)
				if(usr.seer==1)
					...
					usr << SPAN_DANGER("The world beyond fades from your vision.")
					usr.see_invisible = SEE_INVISIBLE_LIVING
					usr.seer = 0
				else if(usr.see_invisible!=SEE_INVISIBLE_LIVING)
					usr << SPAN_WARNING("The world beyond flashes your eyes but disappears quickly, as if something is disrupting your vision.")
					usr.see_invisible = SEE_INVISIBLE_CULT
					usr.seer = 0
				else
					usr.say("Rash'tla sektath mal[pick("'","`")]zua. Zasan therium vivira. Itonis al'ra matum!")
					usr << SPAN_WARNING("The world beyond opens to your eyes.")
					usr.see_invisible = SEE_INVISIBLE_CULT
					usr.seer = 1
				return
			return fizzle()
		...
		raise()
			...
			if(!corpse_to_raise)
				if(is_sacrifice_target)
					usr << SPAN_WARNING("The Geometer of blood wants this mortal for himself.")
				return fizzle()
			...
			if(!body_to_sacrifice)
				if (is_sacrifice_target)
					usr << SPAN_WARNING("The Geometer of Blood wants that corpse for himself.")
				else
					usr << SPAN_WARNING("The sacrifical corpse is not dead. You must free it from this world of illusions before it may be used.")
				return fizzle()
			...
			if(!ghost)
				usr << SPAN_WARNING("You require a restless spirit which clings to this world. Beckon their prescence with the sacred chants of Nar-Sie.")
				return fizzle()
			...
			body_to_sacrifice.visible_message(SPAN_DANGER("[body_to_sacrifice] is torn apart, a black smoke swiftly dissipating from \his remains!"), \
		...
		obscure(var/rad)
			...
			if(S)
				if(istype(src,/obj/effect/rune))
					...
					usr << SPAN_WARNING("Your talisman turns into gray dust, veiling the surrounding runes.")
		...
		talisman()//only hide, emp, teleport, deafen, blind and tome runes can be imbued atm
			...
			if (!newtalisman)
				if (unsuitable_newtalisman)
					usr << SPAN_WARNING("The blank is tainted. It is unsuitable.")
				return fizzle()
...
/////////////////////////////////////////THIRTEENTH RUNE
		...
		mend()
			...
			runedec+=10
			user.visible_message(SPAN_DANGER("\The [user] keels over dead, \his blood glowing blue as it escapes \his body and dissipates into thin air."), \
				...
				if (ticker.mode.name == MODE_CULT)
					if(H.mind == cult.sacrifice_target)
						if(cultsinrange.len >= 3)
							...
							usr << SPAN_WARNING("Your target's earthly bonds are too strong. You need more cultists to succeed in this ritual.")
					else
						if(cultsinrange.len >= 3)
							if(H.stat !=2)
								if(prob(80) || worth)
									...
									usr << SPAN_WARNING("However, this soul was not enough to gain His favor.")
								if(isrobot(H))
									H.dust()//To prevent the MMI from remaining
								else
									H.gib()
								...
								if(prob(40) || worth)
									...
									usr << "<span class='cult'>The Geometer of Blood accepts this sacrifice.</span>"
									usr << SPAN_WARNING("However, a mere dead body is not enough to satisfy Him.")
								if(isrobot(H))
									H.dust()//To prevent the MMI from remaining
								else
									H.gib()
							...
							if(H.stat !=2)
								usr << SPAN_WARNING("The victim is still alive, you will need more cultists chanting for the sacrifice to succeed.")
							else
								if(prob(40))
...
/////////////////////////////////////////SIXTEENTH RUNE
		...
		revealrunes(var/obj/W as obj)
			...
			if(S)
				if(istype(W,/obj/item/weapon/nullrod))
					usr << SPAN_WARNING("Arcane markings suddenly glow from underneath a thin layer of dust!")
					return
				if(istype(W,/obj/effect/rune))
				...
				if(istype(W,/obj/item/weapon/paper/talisman))
					...
					usr << SPAN_WARNING("Your talisman turns into red dust, revealing the surrounding runes.")
...
/////////////////////////////////////////SEVENTEENTH RUNE
		...
		wall()
			...
			if(src.density)
				usr << SPAN_DANGER("Your blood flows into the rune, and you feel that the very space over the rune thickens.")
			else
				usr << SPAN_DANGER("Your blood flows into the rune, and you feel as the rune releases its grasp on space.")
			return
		...
		freedom()
			...
			if(users.len>=3)
				...
				if(!(cultist.buckled || \
					...
					user << SPAN_WARNING("The [cultist] is already free.")
					return
		...
		cultsummon()
			...
			if(users.len>=3)
				...
				if(cultist.buckled || cultist.handcuffed || (!isturf(cultist.loc) && !istype(cultist.loc, /obj/structure/closet)))
					user << SPAN_WARNING("You cannot summon \the [cultist], for \his shackles of blood are strong.")
					return fizzle()
				cultist.loc = src.loc
				cultist.lying = 1
		...
		deafen()
			if(istype(src,/obj/effect/rune))
				...
				if(affected.len)
					...
					usr << SPAN_WARNING("The world becomes quiet as the deafening rune dissipates into fine dust.")
				...
				if(affected.len)
					...
					usr << SPAN_WARNING("Your talisman turns into gray dust, deafening everyone around.")
					admin_attacker_log_many_victims(usr, affected, "Used a deafen rune.", "Was victim of a deafen rune.", "used a deafen rune on")
					for (var/mob/V in orange(1,src))
						if(!(iscultist(V)))
		...
		blind()
			if(istype(src,/obj/effect/rune))
				...
				if(affected.len)
					...
					usr << SPAN_WARNING("The rune flashes, blinding those who not follow the Nar-Sie, and dissipates into fine dust.")
				...
				if(affected.len)
					...
					usr << SPAN_WARNING("Your talisman turns into gray dust, blinding those who not follow the Nar-Sie.")
					admin_attacker_log_many_victims(usr, affected, "Used a blindness rune.", "Was victim of a blindness rune.", "used a blindness rune on")
			return
...
//			var/list/cultboil = list(cultists-usr) //and for this words are destroy-see-blood.
			...
			if(cultists.len>=3)
				for(var/mob/living/carbon/M in viewers(usr))
					...
					M << SPAN_DANGER("Your blood boils!")
					victims += M
					if(prob(5))
						spawn(5)
							M.gib()
					...
					if(R.blood_DNA == src.blood_DNA)
						for(var/mob/living/M in orange(2,R))
							...
							if (R.invisibility>M.see_invisible)
								M << SPAN_DANGER("Aargh it burns!")
							else
								M << SPAN_DANGER("Rune suddenly ignites, burning you!")
				...
				for(var/obj/effect/decal/cleanable/blood/B in world)
					if(B.blood_DNA == src.blood_DNA)
						for(var/mob/living/M in orange(1,B))
							...
							M << SPAN_DANGER("Blood suddenly ignites, burning you!")
