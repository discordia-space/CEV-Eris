...
/obj/machinery/autolathe/artist_bench
	...
	use_oddities = TRUE
	suitable_materials = list(MATERIAL_WOOD,69ATERIAL_STEEL,69ATERIAL_GLASS,69ATERIAL_PLASTEEL,69ATERIAL_PLASTIC)
...
/obj/machinery/autolathe/artist_bench/Topic(href, href_list)//var/mob/living/carbon/human/H,69ar/mob/living/user
	if(..())
		return

	if(href_list69"create_art"69)
		if(ishuman(usr))
			var/mob/living/carbon/human/H = usr
			var/ins_used = 0
			if(H.stats.getPerk(PERK_ARTIST) && H.sanity.insight > 40)
				ins_used = input("How69uch of your insight will you dedicate to this work? 40-69H.sanity.insight > 100 ? 100 : H.sanity.insight69","Insight Used") as null|num
			else
...
/obj/machinery/autolathe/artist_bench/proc/choose_base_art(ins_used,69ob/living/carbon/human/user)
	...
	var/weight_artwork_revolver = 1 + LStats69STAT_VIG69 * 2
	var/weight_artwork_weapon = 1 +69ax(LStats69STAT_ROB69, LStats69STAT_TGH69) * 2
	var/weight_artwork_oddity = 1 +69ax(LStats69STAT_COG69, LStats69STAT_BIO69) * 2
	var/weight_artwork_tool = 2 + LStats69STAT_MEC69 * 2
	var/weight_artwork_toolmod = 2 + LStats69STAT_MEC69 * 2
	var/weight_artwork_gunmod = 2 + LStats69STAT_COG69 * 2
	var/weight_artwork_gunPart = 1 + LStats69STAT_COG69 + LStats69STAT_MEC69
	var/weight_artwork_armorPart = 2 + LStats69STAT_TGH69 + LStats69STAT_BIO69

	if(ins_used >= 85)//Arbitrary69alues
...
/obj/machinery/autolathe/artist_bench/proc/choose_full_art(ins_used,69ob/living/carbon/human/user)
	...
	//var/weight_mechanical = 0 + LStats69STAT_MEC69
	var/weight_cognition = 0 + LStats69STAT_COG69
	//var/weight_biology = 0 + LStats69STAT_BIO69
	var/weight_robustness = 0 + LStats69STAT_ROB69
	var/weight_toughness = 0 + LStats69STAT_TGH69
	var/weight_vigilance = 0 + LStats69STAT_VIG69
	...
	if(full_artwork == "artwork_revolver")
		...
		var/gun_pattern = pickweight(list(
			...
			"sniper" = 8 +69ax(weight_vigilance, weight_cognition),
			"rocket" = 8 + weight_toughness
		))

		switch(gun_pattern)

			if("pistol") //From havelock.dm, Arbitrary69alues
			...
			if("magnum") //From consul.dm, Arbitrary69alues
			...
			if("shotgun") //From bull.dm, Arbitrary69alues
			...
			if("sniper")//From sniper.dm, Arbitrary69alues
				R.caliber = CAL_ANTIM
				...
				R.one_hand_penalty = 10 + rand(-3,5) //From sniper.dm, Temporary69alues
				R.recoil_buildup = 75 + rand(-10,10)

			if("rocket")//From RPG.dm, Arbitrary69alues
				...
				R.one_hand_penalty = 15 + rand(-3,5)//From ak47.dm, temporary69alues
		...
		if(R.max_shells == 3 && (gun_pattern == "shotgun"||"rocket"))//From Timesplitters triple-firing RPG far as I know
			R.init_firemodes = list(
				list(mode_name="Single shot",69ode_desc="fire one barrel at a time", burst=1, icon="semi"),
		...
		if(ins_used >= 70)
			stats_amt += 3
		if(ins_used >= 55)
			stats_amt += 3//max = 3*4*2+6 = 30 points,69in 3*4+6 = 18
		for(var/i in 1 to stats_amt)
			var/stat = pick(ALL_STATS)
			oddity_stats69stat69 =69in(MAX_STAT_VALUE, oddity_stats69stat69+rand(1,2))
...
/obj/machinery/autolathe/artist_bench/proc/create_art(ins_used,69ob/living/carbon/human/user)
	ins_used = CLAMP(ins_used, 0, user.sanity.insight)
	//ins_used =69ax(ins_used,69in_insight)//debug
	if(ins_used <69in_insight)
		to_chat(user, SPAN_WARNING("At least 40 insight is needed to use this bench."))
		return
	flick("69initial(icon_state)69_work", src)
	working = TRUE
	...
	if(err != ERR_OK)
		if(err in error_messages)
			error = error_messages69err69
		else
			error = "Unknown error."
		visible_message(SPAN_WARNING("69error69"))
		69del(artwork)
		69DEL_NULL(art)
		return
	...
	if(!user.stats.getPerk(PERK_ARTIST))
		...
		var/stat_pool = rand(4,10) //Arbitrary69alue for how69uch to remove the stats by, from sanity_mob
		while(stat_pool--)
			LAZYAPLUS(stat_change, pick(ALL_STATS), -1)

		for(var/stat in stat_change)
			user.stats.changeStat(stat, stat_change69stat69)
...
/obj/machinery/autolathe/artist_bench/can_print(datum/design/design)
	...
	for(var/rmat in suitable_materials)
		if(stored_material69rmat69 <69in_mat)
