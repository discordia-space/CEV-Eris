...
/obj/machinery/power/supply_beacon/Process()
	...
	if(!target_drop_time)
		...
		var/drop_y = src.y-2
		var/drop_z = src.z
		command_announcement.Announce("Nyx Rapid Fabrication priority supply request #[rand(1000,9999)]-[rand(100,999)] recieved. Shipment dispatched via ballistic supply pod for immediate delivery. Have a tramadol day.", "Thank You For Your Patronage")
		spawn(rand(100,300))
