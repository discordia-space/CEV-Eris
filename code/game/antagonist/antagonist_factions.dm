...
/datum/faction/proc/communicate(mob/living/carbon/human/user)
	if(!is_member(user) || user.stat != CONSCIOUS)
		return

	if(user.handcuffed && istype(user.handcuffed, /obj/item/handcuffs/hitech))
		user.electrocute_act(10, user.handcuffed)
		return
