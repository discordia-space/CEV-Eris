...
/obj/machinery/mecha_part_fabricator/proc/get_build_options(var/mob/user)
	...
	for(var/i = 1 to files.known_designs.len)
		...
		if(!D.build_path || !(D.build_type & MECHFAB))
			continue
		. += list(list("name" = D.name, "id" = i, "category" = D.category, "resourses" = get_design_resourses(D), "time" = get_design_time(D), "icon" = SSdynamic_cache.getCacheFilename(D.build_path)))
