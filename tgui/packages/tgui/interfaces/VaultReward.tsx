...
export const VaultReward = (props: any, context: any) => {
  ...
  const handleItemClick = (type_to_check: string) => {
    if (
      data.equipped_items.some(
        (item, _, __) => item.item_type === type_to_check
      )
    ) {
      act('deselect_type', { type: type_to_check });
    } else if (
      data.items.some((item, _, __) => item.item_type === type_to_check)
    ) {
  ...
  if (searchQuery !== null) {
    itemsToDisplay = itemsToDisplay.filter(
      (vault_item, _) =>
        vault_item.name.search(searchQuery || vault_item.name) >= 0
    );
        ...
        <Section title="User Info">
          <Table>
            ...
            <Table.Row className="candystripe" key>
              <Table.Cell textAlign="center">{data.ckey}</Table.Cell>
