			...
			<Stack vertical>
				<Stack.Item>
					{mutation.Source === 'disk' && (
						<MutationCombiner
							disabled={
								!hasDisk || diskCapacity <= 0 || diskReadOnly
							}
				...
				<Stack.Item>
					{['occupant', 'disk', 'console'].includes(
						mutation.Source,
					) && (
						<Stack vertical>
							<Stack.Item>
								<Dropdown
									width="240px"
									options={advInjectors.map(
										(injector) => injector.name,
									)}
									disabled={
										advInjectors.length === 0 ||
										!mutation.Active
									}
				...
				<Stack.Item>
					<Stack>
						{['disk', 'occupant'].includes(mutation.Source) && (
							<Stack.Item>
								<Button
									icon="save"
									disabled={
										savedToConsole || !mutation.Active
									}
						...
						{['console', 'disk', 'injector'].includes(
							mutation.Source,
						) && (
							<Stack.Item>
								<Button
