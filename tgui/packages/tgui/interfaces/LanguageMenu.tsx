...
const LangUnderstandIcon = (props: LanguageProps) => {
	...
	if (!language.can_understand && language.partial_understanding > 0) {
		return (
			<Tooltip
				content={`You can only partially understand ${language.name}.`}
			>
				<Box
					inline
					style={{
	...
	return (
		<Icon name="brain" color={language.can_understand ? 'good' : 'bad'} />
	);
};
		...
		<Table.Row className="candystripe">
			...
			{!!admin_mode && (
				<Table.Cell>
					...
					<Button
						disabled={
							!language.can_speak && !language.can_understand
						}
						onClick={() =>
							act('remove_language', {
								language_name: language.name,
