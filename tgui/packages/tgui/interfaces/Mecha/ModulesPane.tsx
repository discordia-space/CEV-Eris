...
export const ModuleDetails = (props) => {
	...
	return (
		<Box>
			...
			<Section>
				{snowflake &&
				snowflake.snowflake_id === MECHA_SNOWFLAKE_ID_EJECTOR ? (
...
const ModuleDetailsBasic = (props) => {
	...
	return (
		<>
			...
			{!weapons_safety &&
				['mecha_l_arm', 'mecha_r_arm'].includes(slot) && (
		...
		<>
			...
			{!!projectiles_cache_max && (
				<LabeledList.Item label="Stored">
					<ProgressBar
						value={projectiles_cache / projectiles_cache_max}
					>
		...
		<Box>
			...
			>
				<LabeledList.Item label="Direction">
					<Button
						content={
							tank_pump_direction ? 'Area → Tank' : 'Tank → Area'
						}
						onClick={() =>
							act('equip_act', {
								ref: ref,
				...
				<LabeledList.Item label="Target Pressure">
					...
					<Button
						icon="sync"
						disabled={
							tank_pump_pressure === Math.round(one_atmosphere)
						}
						onClick={() =>
							act('equip_act', {
								ref: ref,
