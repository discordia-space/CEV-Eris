							...
							>
								{Array.from(
									Object.keys(item.reqs).map((atom_id) => {
										const name =
											data.atom_data[(atom_id as any) - 1]
												?.name;
										const is_reagent =
											data.atom_data[(atom_id as any) - 1]
												?.is_reagent;
							...
							<Box style={{ textTransform: 'capitalize' }}>
								...
								{item.chem_catalysts && (
									<Box>
										...
										{Object.keys(item.chem_catalysts).map(
											(atom_id) => (
												<AtomContent
													key={atom_id}
													atom_id={atom_id}
