...
export const RequestManager = (props) => {
	...
	if (searchText.length > 0) {
		displayedRequests = displayedRequests.filter((request) =>
			search(request),
		);
	}
...
const RequestControls = (props) => {
	...
	return (
		<div className="RequestManager__controlsContainer">
			...
			<Button onClick={() => act('logs', { id: request.id })}>
				LOGS
			</Button>
			<Button onClick={() => act('smite', { id: request.id })}>
				SMITE
			</Button>
			{request.req_type !== 'request_prayer' && (
				<Button onClick={() => act('rply', { id: request.id })}>
					RPLY
				</Button>
			)}
			...
			{request.req_type === 'request_fax' && (
				<>
					<Button onClick={() => act('show', { id: request.id })}>
						SHOW
					</Button>
			...
			{request.req_type === 'request_internet_sound' && (
				<Button onClick={() => act('play', { id: request.id })}>
					PLAY
				</Button>
			)}
		</div>
