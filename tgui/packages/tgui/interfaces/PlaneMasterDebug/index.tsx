	...
	return (
		...
		19 *
			Math.max(
				...
				plane.outgoing_filters.length +
					plane.outgoing_relays.length +
					1,
			) +
		15
	);
...
function mapPlanes(planes: PlaneData[]) {
	...
	for (let k = 0; k < stackKeys.length; k++) {
		...
		for (let i = 0; i < stack.length; i++) {
			...
			const desiredPos = getDesiredPlanePosition(
				plane,
				+key,
				tallestStack,
			);
