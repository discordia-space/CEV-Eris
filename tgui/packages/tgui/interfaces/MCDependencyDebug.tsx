...
export function MCDependencyDebug(props) {
	...
	useLayoutEffect(() => {
		...
		for (let i = 0; i < subsystems.length; i++) {
			...
			if (
				domElements.input === undefined ||
				domElements.output === undefined
			) {
				continue;
			}
					...
					{subsystems.map((subsystem, y) => (
						<GraphNode
							...
							inputRef={(element) => {
								...
								connectionDom.current[subsystem.name].input =
									element;
							}}
							outputRef={(element) => {
								if (
								...
								connectionDom.current[subsystem.name].output =
									element;
							}}
						/>
					))}
