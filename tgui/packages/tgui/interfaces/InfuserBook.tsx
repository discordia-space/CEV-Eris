...
export const InfuserBook = (props) => {
	...
	const setPage = (newPage) => {
		...
		if (newPage < 0) {
			...
			newBookPosition.pageInChapter =
				paginatedEntries[chapter - 1].length - 1;
						...
						<Stack fontSize="18px" fill>
							...
							<Stack.Item grow={1}>
								<Section fitted fill pt="3px">
									...
									{paginatedEntries[chapter].length +
										(chapter === 0 ? 1 : 0)}
								</Section>
							</Stack.Item>
							<Stack.Item grow={2}>
		...
		<Section title="DNA Infusion Guide" height={PAGE_HEIGHT}>
			<Stack vertical>
				...
				<Stack.Item color="label">
					DNA Infusion is the practice of integrating dead creature
					DNA into yourself, mutating one of your organs into a
					genetic slurry that sits somewhere between being yours or
					the creature&apos;s. While this does bring you further away
					from being human, and gives a slew of... unfortunate side
					effects, it also grants new capabilities.{' '}
					<b>
						Above all else, you have to understand that gene-mutants
						are usually very good at specific things, especially
						with their threshold bonuses.
					</b>
				</Stack.Item>
				...
				<Stack.Item color="label">
					1. Load a dead creature into the machine. This is what
					you&apos;re infusing from.
					<br />
					...
					<Box mt="10px" inline color="white">
						And you&apos;re done! Note that the infusion source will
		...
		>
			<Stack vertical fill>
				<Stack.Item>
					<BlockQuote>
						...
						{entry.threshold_desc && (
							<>
								If a subject infuses with enough DNA,{' '}
								{entry.threshold_desc}
							</>
						)}
					</BlockQuote>
				...
				<Stack.Item>
					...
					<Box
						inline
						color={entry.name === 'Rejected' ? 'red' : 'green'}
					>
