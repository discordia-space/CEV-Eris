	...
	return (
		<>
			<Section title="Submission Form">
				{fileList.length === 0 && (
					<NoticeBox>
						Use data disk to download files from compressor or
						doppler array.
					</NoticeBox>
				)}
				<LabeledList>
					...
					>
						<Box position="relative" top="8px">
							<Dropdown
								width="100%"
								options={allowedTiers.map((number) =>
									String(number),
								)}
		...
		<>
			{!has_techweb && (
				<Section title="No techweb detected!" key="rewards">
					Please sync this application to a valid techweb to upload
					progress!
				</Section>
			)}
