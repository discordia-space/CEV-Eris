	...
	const faxes = data.faxes
		? sortBy(
				data.syndicate_network
					? data.faxes.filter(
							(filterFax: FaxInfo) => filterFax.visible,
						)
					: data.faxes.filter(
							(filterFax: FaxInfo) =>
								filterFax.visible &&
								!filterFax.syndicate_network,
						),
					...
					<Table>
						<Table.Cell>
							{data.fax_history !== null
								? data.fax_history.map(
										(history: FaxHistory) => (
											<Table.Row
												key={history.history_type}
											>
												{
													<Box
														color={
