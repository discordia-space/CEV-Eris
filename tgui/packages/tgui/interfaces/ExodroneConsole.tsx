...
const SignalLostModal = (props) => {
	...
	return (
		...
		>
			...
			<Box p={2} italic>
				Connection to exploration drone interrupted. Please contact
				nearest Nanotrasen Exploration Division representative for
			...
			<Stack.Item grow>
				<ProgressBar
					...
					ranges={{
						...
						average: [
							0.4 * drone_max_integrity,
							0.7 * drone_max_integrity,
						],
					...
					<Section
						...
						buttons={
							<Box>
								ETA:{' '}
								{formatTime(
									site.distance * drone_travel_coefficent,
									'short',
								)}
								<Button
									ml={1}
				...
				{valid_destinations?.map((destination) => (
					<Section
						...
						buttons={
							<>
								ETA:{' '}
								{formatTime(travel_cost(destination), 'short')}
								<Button
									ml={1}
		...
		>
			<Stack vertical fill>
				...
				<Stack.Item align="center" grow>
					<Button onClick={() => setTravelDimmerShown(true)}>
						Travel
					</Button>
				</Stack.Item>
			</Stack>
