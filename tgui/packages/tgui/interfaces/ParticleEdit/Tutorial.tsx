...
const VarExplanation = {
	...
	motion: {
		desc: (
			<>
				The first thing to understand before we dive into how generators
				is motion. These basics assume you fell asleep during physics
				class so you can skip this if you didnt.
				<br />
				<br />
				First things off there are 3 variable you care about: POSITION,
				VELOCITY and ACCELERATION. Position is where you are in a 3D
				space, velocity is how fast you are moving in x/y/z directions,
				and acceleration is how much velocity is being changed by.
				Shortly summarized this is
				<br />
				<br />
				...
				<br />
				<b>
					location = initial location + (initial velocity * time) +
					(0.5 * acceleration * time<sup>2</sup>)
				</b>
				<br />
				<br />
				...
				<b>
					...
					* (average_drift_during_ticks + gravity - (velocity *
					friction
					<sup>ticks_elapsed</sup>)) * ticks_elapsed<sup>2</sup>)
				</b>
				<br />
				<br />
				Now while this is all nice and dandy how does it look like in
				practice? If you look at only one direction then movement will
				look like this:
				<br />
	...
	randtypes: {
		desc: (
			<Stack vertical fill>
				<Stack.Item>
					If you didnt sleep during statistics class you can skip this
					part. Basically the type of randomness you choose determines
					the probability curve (if you take a million samples how
					will they look on a chart) of the values to pick. The
					probability graphs for the byond rands are:
				</Stack.Item>
				<Stack.Item>
					<Stack>
						<Stack.Item>
							<Stack vertical fill>
								<Stack.Item bold>
									UNIFORM RAND(default):
								</Stack.Item>
								<Stack.Item>
									<Image
										width={25}
										src={resolveAsset('uniform')}
									/>
								</Stack.Item>
							</Stack>
						</Stack.Item>
			...
			<Stack vertical fill>
				...
				<Stack.Item height={7}>
					<Stack>
						...
						<Stack.Item width={11}>vector</Stack.Item>
						<Stack.Item width={20}>
							A random vector within a box whose corners are at A
							and B.
						</Stack.Item>
						<Stack.Item>
							<Image
				...
				<Stack.Item height={7}>
					<Stack>
						...
						<Stack.Item width={20}>
							Result type depends on whether A or B are matrices
							or not. The result is interpolated between A and B;
				...
				<Stack.Item height={6}>
					<Stack>
						...
						<Stack.Item width={11}>vector</Stack.Item>
						<Stack.Item width={20}>
							A random XY-only vector in a ring between radius A
							and B, centered at 0,0.
						</Stack.Item>
						<Stack.Item>
							<Image
				...
				<Stack.Item height={6}>
					<Stack>
						...
						<Stack.Item width={11}>vector</Stack.Item>
						<Stack.Item width={20}>
							A random vector in a spherical shell between radius
							A and B, centered at 0,0,0.
						</Stack.Item>
						<Stack.Item>
							<Image
				...
				<Stack.Item height={6}>
					<Stack>
						...
						<Stack.Item width={11}>vector</Stack.Item>
						<Stack.Item width={20}>
							A random XY-only vector between squares of sizes A
							and B. (The length of the square is between A*2 and
							B*2, centered at 0,0.)
						</Stack.Item>
						<Stack.Item>
							<Box width={15} />
				...
				<Stack.Item height={10}>
					<Stack>
						...
						<Stack.Item width={11}>vector</Stack.Item>
						<Stack.Item width={20}>
							A random vector between cubes of sizes A and B. (The
							length of the cube is between A*2 and B*2, centered
							at 0,0,0.)
						</Stack.Item>
						<Stack.Item>
							<Image
