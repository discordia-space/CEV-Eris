...
export class Uplink extends Component<{}, UplinkState> {
	...
	render() {
		...
		const { allItems, allCategories, currentTab } = this
			.state as UplinkState;
		...
		for (let i = 0; i < itemsToAdd.length; i++) {
			...
			const hasEnoughPop =
				!joined_population ||
				joined_population >= item.population_minimum;
			...
			const canBuy =
				telecrystals >= item.cost && (stock === null || stock > 0);
			items.push({
				id: item.id,
				name: item.name,
				icon: item.icon,
				...
				desc: (
					<>
						...
						{(item.lock_other_purchases && (
							<NoticeBox mt={1}>
								Taking this item will lock you from further
								purchasing from the marketplace. Additionally,
								if you have already purchased an item, you will
								not be able to purchase this.
							</NoticeBox>
						)) ||
							null}
					</>
