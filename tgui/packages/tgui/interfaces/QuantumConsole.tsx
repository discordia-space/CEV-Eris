...
function DomainEntry(props: DomainEntryProps) {
	...
	return (
		<Collapsible
			buttons={
				<Tooltip
					content={!!generated_domain && 'Stop current domain first.'}
				>
					<Button
						disabled={
							!!generated_domain ||
...
const AvatarDisplay = (props) => {
	...
	return (
		...
		>
			<Table>
				{avatars.map(
					({ health, name, pilot, brute, burn, tox, oxy }) => (
						<Table.Row key={name}>
