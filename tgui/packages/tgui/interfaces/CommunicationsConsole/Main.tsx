				...
				<Section title="Emergency Shuttle">
					...
					{!!shuttleCalledPreviously &&
						(shuttleLastCalled ? (
							...
							<Box>
								Unable to trace most recent shuttle/recall
								signal.
							</Box>
						))}
				</Section>
			...
			<Section title="Functions">
				<Flex direction="column">
					...
					{!!canToggleEmergencyAccess && (
						...
						>
							{emergencyAccess ? 'Disable' : 'Enable'} Emergency
							Maintenance Access
						</Button.Confirm>
					)}
					...
					{!syndicate && (
						<Button
							...
							onClick={() =>
								act('setState', {
									state: ShuttleState.CHANGING_STATUS,
								})
							}
						>
							Set Status Display
					...
					<Button
						icon="envelope-o"
						onClick={() =>
							act('setState', { state: ShuttleState.MESSAGES })
						}
					>
						Message List
					</Button>
					...
					{canBuyShuttles !== 0 && (
						<Button
							...
							onClick={() =>
								act('setState', {
									state: ShuttleState.BUYING_SHUTTLE,
								})
							}
						>
							Purchase Shuttle
						...
						{sectors.map((sectorName) => (
							<Flex.Item key={sectorName}>
								<Button
									disabled={!importantActionReady}
									onClick={() =>
										setMessagingSector(sectorName)
									}
								>
									Send a message to station in {sectorName}{' '}
									sector
								</Button>
							</Flex.Item>
						))}
