			...
			<Stack.Item>
				<Section>
					...
					<Button
						content={'Logout'}
						onClick={() =>
							act('auth', { auth_password: auth_password })
						}
					/>
					<Button
						...
						<>
							...
							<Stack.Item grow>
								{(screen === Screen.Main && <MainScreen />) ||
									(screen === Screen.MessageLogs && (
										<MessageLogsScreen />
									)) ||
									(screen === Screen.RequestLogs && (
										<RequestLogsScreen />
									)) ||
									(screen === Screen.Hacked && (
										<HackedScreen />
									))}
							</Stack.Item>
							<Stack.Item>
							...
							<label>
								Reg. #514 forbids sending messages to a Head of
								Staff containing Erotic Rendering Properties.
							</label>
						</>
					) : (
						<>
							<Stack.Item>
								<NoticeBox color="red">
									Server not found, click button to scan the
									network
								</NoticeBox>
							</Stack.Item>
							<Stack.Item>
