		...
		return cell_line.type !== 'virus' ? (
			<Stack key={cell_line.desc} mt={2}>
				...
				<Stack.Item grow pl={1}>
					...
					>
						<Box my={1}>
							Consume {cell_line.consumption_rate} units of every
							nutrient per second to grow by{' '}
							{cell_line.growth_rate}%.
						</Box>
						{cell_line.suspectibility > 0 && (
							<Box my={1}>
								Reduced by {cell_line.suspectibility}% when
						...
						<Stack fill>
							<Stack.Item grow>
								...
								{Object.keys(cell_line.requireds).map(
									(reagent) => (
							...
							<Stack.Item grow>
								...
								{Object.keys(cell_line.supplementaries).map(
									(reagent) => (
										<Button
											fluid
											color="good"
							...
							<Stack.Item grow>
								...
								{Object.keys(cell_line.suppressives).map(
									(reagent) => (
										<Button
											fluid
											color="bad"
			...
			<Stack key={cell_line.desc} mt={2}>
				...
				<Stack.Item grow pl={1}>
					<Section title={cell_line.desc}>
						<Box my={1}>
							Reduces growth of other cell lines when not
							suppressed by Spaceacillin.
						</Box>
					</Section>
				</Stack.Item>
			</Stack>
