...
const SupermatterEntry = (props: SupermatterEntryProps) => {
	...
	return (
		<>
			...
			{activeDetail && !!detail && (
				<Stack.Item pl={3}>{detail}</Stack.Item>
			)}
		</>
	);
};
					...
					<Stack vertical>
						{gas_composition.map(([gas_path, amount]) => (
							<SupermatterEntry
								key={gas_path}
								title={
									getGasFromPath(gas_path)?.label || 'Unknown'
								}
								content={
									<ProgressBar
								...
								detail={
									gas_metadata[gas_path] ? (
										<>
											{gas_metadata[gas_path].desc && (
												<br />
											)}
											{gas_metadata[gas_path].numeric_data
												.length ? (
												<>
													<Box mb={1}>
														At{' '}
