			...
			<NtosWindow.Content scrollable>
				<Section>
					<FileTable
						...
						onUpload={(file) =>
							act('PRG_copytousb', { name: file })
						}
						onDelete={(file) =>
							act('PRG_deletefile', { name: file })
						}
						...
						onToggleSilence={(file) =>
							act('PRG_togglesilence', { name: file })
						}
					/>
				</Section>
				{usbconnected && (
					<Section title="Data Disk">
						<FileTable
							...
							usbconnected={usbconnected}
							onUpload={(file) =>
								act('PRG_copyfromusb', { name: file })
							}
							onDelete={(file) =>
								act('PRG_usbdeletefile', { name: file })
							}
							onRename={(file, newName) =>
				...
				<Table.Row key={file.name} className="candystripe">
					...
					<Table.Cell collapsing>
						{!!file.alert_able && (
							<Button
								icon={
									file.alert_silenced ? 'bell-slash' : 'bell'
								}
