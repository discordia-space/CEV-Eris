		...
		<Window title="Ore Redemption Machine" width={435} height={500}>
			<Window.Content>
				<Stack fill vertical>
					...
					<NoticeBox info mb={0}>
						This machine only accepts ore. Gibtonite and Slag are
						not accepted.
					</NoticeBox>
					<Stack.Item mb={-1}>
		...
		<Table.Row className="candystripe">
			...
			<Table.Cell collapsing textAlign="left">
				<Button
					color="transparent"
					tooltip={
						material.value ? material.value + ' cr' : 'No cost'
					}
					onClick={() => onRelease(1)}
				>
					x1
				...
				<Button
					...
					tooltip={
						material.value
							? material.value * print_amount + ' cr'
							: 'No cost'
					}
...
function MaterialSection(props: MaterialSectionProps) {
	...
	const material_filtered =
		searchItem.length > 0
			...
			: materials.filter(
					(material) => material && material.category === tab,
				);
