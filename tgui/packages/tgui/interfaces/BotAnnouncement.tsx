...
export const BotAnnouncement = (props) => {
	...
	let filteredShortcuts: ButtonDataWithId[] = button_data.map(
		(val, index) => ({
			button: val,
			index,
		}),
							...
							{filteredLines.map((val) => (
								<Stack.Item key={val.string}>
									<Button
										py={1}
										color={
											selectedLine === val.index
												? 'green'
												: 'transparent'
										}
										fluid
										onClick={() =>
											setSelectedLine(val.index)
										}
										minHeight="32px"
									>
										<Stack>
