...
const InstrumentSettings = (props) => {
	...
	const instrument_id_by_name = (name) => {
		return possible_instruments.find(
			...
			<Stack mt={1}>
				...
				<Stack.Item>
					...
					<Box>
						<Button
							onClick={() =>
								act('toggle_sustain_hold_indefinitely')
							}
						>
							{sustain_indefinitely
...
const EditingSettings = (props) => {
	...
	return (
		<Section>
			<Box>
				<Button onClick={() => act('start_new_song')}>
					Start a New Song
				</Button>
				<Button onClick={() => act('import_song')}>
					Import a Song
				</Button>
			</Box>
			<Box>
				...
				<Button
					onClick={() =>
						act('tempo', { tempo_change: 'increase_speed' })
					}
				>
					-
				</Button>{' '}
				...
				<Button
					onClick={() =>
						act('tempo', { tempo_change: 'decrease_speed' })
					}
				>
					+
				</Button>
...
const HelpSection = (props) => {
	...
	return (
		<Section>
			<Box>
				Lines are a series of chords, separated by commas (,), each with
				notes separated by hyphens (-).
				<br />
				Every note in a chord will play together, with chord timed by
				the tempo.
				<br />
				Notes are played by the names of the note, and optionally, the
				accidental, and/or the octave number.
				<br />
				By default, every note is natural and in octave 3. Defining
				otherwise is remembered for each note.
				<br />
				...
				<br />
				Chords can be played simply by seperating each note with a
				hyphon: <i>A-C#,Cn-E,E-G#,Gn-B</i>
				<br />A pause may be denoted by an empty chord: <i>C,E,,C,G</i>
				<br />
				To make a chord be a different time, end it with /x, where the
				chord length will be length
				<br />
