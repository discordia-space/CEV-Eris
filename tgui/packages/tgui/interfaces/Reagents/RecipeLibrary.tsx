			...
			buttons={
				<>
					...
					<Button
						...
						onClick={() =>
							setPage(Math.min(page + 1, pageIndexMax))
						}
					/>
				</>
			}
					...
					.map((reaction) => (
						<Table.Row key={reaction.id} className="candystripe">
							...
							<Table.Cell width="60px">
								{flagIcons
									.filter(
										(meta) => reaction.bitflags & meta.flag,
									)
									.map((meta) => (
										<Icon
											key={meta.flag}
							...
							<Table.Cell width="20px">
								{!bookmarkMode ? (
									<Button
										icon="book"
										color="green"
										disabled={bookmarkedReactions.has(
											reaction,
										)}
										onClick={() => {
									...
									<Button
										...
										color="red"
										onClick={() =>
											bookmarkedReactions.delete(reaction)
										}
									/>
								)}
							</Table.Cell>
