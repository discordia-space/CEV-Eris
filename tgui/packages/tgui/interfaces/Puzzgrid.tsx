...
export const Puzzgrid = (props) => {
	...
	const answersLeft = data.answers.filter(
		(answer) =>
			!data.solved_groups.find(
				(group) => group.answers.indexOf(answer) !== -1,
			),
	);
		...
		>
			<Window.Content>
				<Stack vertical fill>
					...
					{range(0, answersLeft.length / CELLS_PER_GROUP).map(
						(row) => (
							<Stack.Item key={row} grow>
								<Stack fill>
									{range(0, CELLS_PER_GROUP).map((column) => {
										const answer =
											answersLeft[
												row * CELLS_PER_GROUP + column
											];
										const selected =
											data.selected_answers.indexOf(
												answer,
											) !== -1;
										...
										return (
											...
											>
												<PuzzgridButton
													disabled={
														!!data.wrong_group_select_cooldown
													}
													selected={selected}
													onClick={() =>
														act(
